...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
	...
	if (((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) && (bDynamicJoin == false) && (m_dwPersonalPlayCount != 0))
	{
		SendRequestFailure(pIServerUserItem, TEXT("房卡游戏已经开始 ， 现在不能进入该房卡游戏！"), REQUEST_FAILURE_NORMAL);
		return false;
	}
