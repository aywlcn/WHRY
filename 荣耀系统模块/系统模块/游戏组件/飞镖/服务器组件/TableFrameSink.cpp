...
void Debug(char *text,...)
{
	...
	if(num == 0)	
	{		
		fp=fopen("飞镖游戏库存.log","w");	
	}	
	else	
	{		
		fp=fopen("飞镖游戏库存.log","a");	
	}	
	if(fp == NULL)	
		return ;	
	num++;	
...
bool CTableFrameSink::OnEventGameStart()
{
	...
	WriteInfo(TEXT("飞镖游戏库存日志.log"), strStorage);
...
bool CTableFrameSink::OnEventSendGameScene(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
	...
	case GAME_SCENE_PLACE_JETTON:		//下注状态
	case GAME_SCENE_DART:				//射飞镖状态
	...
	case SUB_C_UPDATE_STORAGE:		//更新库存
		{
			...
			if (pUpdateStorage->cbReqType==RQ_SET_STORAGE)
			{
				...
				WriteInfo(TEXT("飞镖游戏控制信息.log"),strControlInfo);
			}
