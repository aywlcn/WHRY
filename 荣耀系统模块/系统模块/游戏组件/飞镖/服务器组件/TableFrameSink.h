...
protected:
	...
	WORD							m_wSitDownInfo[SEAT_COUNT];					//座位号对应的椅子号
...
protected:						
	...
	LONGLONG						m_nBankerTimeLimit;						//次数限制
	LONGLONG						m_nBankerTimeAdd;						//增加次数 (金币大于其他玩家时)
	LONGLONG						m_lExtraBankerScore;					//额外条件 (大于此值时可以无视条件额外坐庄)
	LONGLONG						m_nExtraBankerTime;						//额外次数

	//库存控制
	LONGLONG						m_lStorageStart;						//库存数值
	LONGLONG						m_lStorageCurrent;						//库存数值
	LONGLONG						m_nStorageDeduct;						//扣取比例
	...
	//时间控制
	LONGLONG						m_nFreeTime;							//空闲时间
	LONGLONG						m_nPlaceJettonTime;						//下注时间
	LONGLONG						m_nDartTime;							//射飞镖时间
	LONGLONG						m_nGameEndTime;							//结束时间

	//赔率
	tagOdds							m_tOdds;

	//机器人控制
	LONGLONG						m_nMaxChipRobot;						//最大数目 (下注机器人)
	LONGLONG						m_nChipRobotCount;						//人数统计 (下注机器人)
	LONGLONG						m_lRobotAreaLimit;						//区域统计 (机器人)
	LONGLONG						m_lRobotAreaScore[AREA_COUNT+1];		//区域统计 (机器人)
	LONGLONG						m_nRobotListMaxCount;					//最多人数
...
protected:
	...
	BYTE							m_cbDartPoint[2];						//飞镖点数
	WORD							m_cbDartFlag[2];						//发射标记，是否已发射

	//状态变量
protected:
	DWORD							m_dwJettonTime;							//开始时间
	bool							m_bExchangeBanker;						//变换庄家
	LONGLONG						m_wAddTime;								//额外坐庄 
...
protected:
	...
	BYTE							m_cbWinArea[AREA_COUNT];				//赢的区域
	Region						    m_rgArea[AREA_COUNT];					//记录区域信息
