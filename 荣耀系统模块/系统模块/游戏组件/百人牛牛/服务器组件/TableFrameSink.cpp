...
// 	cbMultiple[4] = HuangMultiple;
	...
	for (WORD _index = ID_TIAN_MEN; _index <= ID_HUANG_MEN;_index++)
	{
		BYTE cbCardType = CT_ERROR;
		if (bWinFlag[_index] == true) //闲家赢
			cbCardType = m_GameLogic.GetCardType(m_cbTableCardArray[_index], MAX_CARD);
		else
			cbCardType = m_GameLogic.GetCardType(m_cbTableCardArray[BANKER_INDEX], MAX_CARD);
		if (cbCardType >= CT_SPECIAL_NIUNIU)
			cbMultiple[_index] = 3;
		else if (cbCardType >= CT_SPECIAL_NIU8)
			cbMultiple[_index] = 2;
	}
...
void CTableFrameSink::ControlWinAerea()
{
	if (m_wCheatArea == 255 || m_wCheatArea < 1 || m_wCheatArea > 4) return;
	BYTE lMultiple = 1;
	if (m_GameLogic.CompareCard(m_cbTableCardArray[BANKER_INDEX], 5, m_cbTableCardArray[m_wCheatArea], 5, lMultiple) == 1)
	{
