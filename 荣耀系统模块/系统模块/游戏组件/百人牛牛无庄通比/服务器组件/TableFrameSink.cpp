...
LONGLONG CTableFrameSink::CalculateScoreEx()
{
	...
	ZeroMemory(m_rgArea, sizeof(m_rgArea));
	//玩家下注
// 	LONGLONG const pUserScore[] = { NULL, m_lAllJettonScore[ID_TIAN_MEN], m_lAllJettonScore[ID_DI_MEN], m_lAllJettonScore[ID_XUAN_MEN],
// 		m_lUserJettonScore[ID_HUANG_MEN] };
	LONGLONG lTianScore = m_lAllJettonScore[1];
	LONGLONG lDiScore = m_lAllJettonScore[2];
	LONGLONG lXuanScore = m_lAllJettonScore[3];
	LONGLONG lHuangScore = m_lAllJettonScore[4];
	LONGLONG lAllScore = lTianScore + lDiScore + lXuanScore + lHuangScore;

	// 赋值
	for (int i = 0; i < 4; i++)
	{
		m_rgArea[i].nIndex = i;
		m_rgArea[i].sCoinScore = 0;
		m_rgArea[i].sBetScore = m_lAllJettonScore[i + 1];
		CopyMemory(m_rgArea[i].bCard, m_cbTableCardArray[i + 1], sizeof(m_cbTableCardArray[i + 1]));
	}
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = i + 1; j < 4; j++)
		{
			if (m_GameLogic.CompareCard(m_rgArea[i].bCard, 5, m_rgArea[j].bCard, 5, bMultiple) == 1)
			{
				rg.nIndex = m_rgArea[i].nIndex;
				rg.sCoinScore = m_rgArea[i].sCoinScore;
				rg.sBetScore = m_rgArea[i].sBetScore;
				CopyMemory(rg.bCard, m_rgArea[i].bCard, sizeof(m_rgArea[i].bCard));

				m_rgArea[i].nIndex = m_rgArea[j].nIndex;
				m_rgArea[i].sCoinScore = m_rgArea[j].sCoinScore;
				m_rgArea[i].sBetScore = m_rgArea[j].sBetScore;
				CopyMemory(m_rgArea[i].bCard, m_rgArea[j].bCard, sizeof(m_rgArea[j].bCard));

				m_rgArea[j].nIndex = rg.nIndex;
				m_rgArea[j].sCoinScore = rg.sCoinScore;
				m_rgArea[j].sBetScore = rg.sBetScore;
				CopyMemory(m_rgArea[j].bCard, rg.bCard, sizeof(rg.bCard));
			}
		}
	}
	...
	for (int i = 0; i < 4; i++)
	{
		m_rgArea[i].sCoinScore = __min(lAllScore, m_rgArea[i].sBetScore * 2);
		lAllScore = __max(0, lAllScore - m_rgArea[i].sCoinScore);
		m_rgArea[i].sCoinScore = m_rgArea[i].sCoinScore - m_rgArea[i].sBetScore;
	}
	...
	for (int i = 0; i < 4; i++)
	{
		for (WORD wChairID = 0; wChairID < GAME_PLAYER; wChairID++)
		{
			...
			if (m_rgArea[i].sCoinScore != 0)
			{
				m_lUserWinScore[wChairID] += (m_lUserJettonScore[m_rgArea[i].nIndex + 1][wChairID]*1.0 / m_rgArea[i].sBetScore) * m_rgArea[i].sCoinScore;
				m_lUserReturnScore[wChairID] += m_lUserJettonScore[m_rgArea[i].nIndex + 1][wChairID];
			}
			else
			{
				m_lUserWinScore[wChairID] += 0;                                  
...
{
	if (m_wCheatArea == 255 || m_wCheatArea < 0 || m_wCheatArea > 4) return;
