...
CTableFrameSink::CTableFrameSink()
{
  ...
  m_wTurnCount = 0;				//下注轮数
  m_wMinLookTurn = 2;				//必闷轮数
  m_wMinCompareTurn = 3;			//比牌轮数
  m_wMaxOpenTurn = 15;				//封顶轮数

  m_bCompareSmall = FALSE;			//比小
  m_cbCompareWay = 0;				//比牌方式比大比小

  m_bSpecilBaozi = TRUE;
  m_cbCompareDouoble = 0;
  ...
  for (WORD i = 0; i < GAME_PLAYER;i++)
	  ZeroMemory(m_fLocation[i], sizeof(m_fLocation[i]));
  ...
  m_hControlInst = LoadLibrary(TEXT("ZhaJinHuaServerControl.dll"));
  if(m_hControlInst)
  {
    typedef void * (*CREATE)();
...
bool  CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
  ...
  //m_pGameServiceOption->wMaxPlayer = m_wPlayerCount;
  ...
  m_bCompareSmall = pGameCustomRule->bCompareSmall;
  m_wMaxOpenTurn = pGameCustomRule->wMaxOpenTurn;
  m_wMinCompareTurn = pGameCustomRule->wMinCompareTurn;
  m_wMinLookTurn = pGameCustomRule->wMinLookTurn;
  m_cbCompareWay = pGameCustomRule->cbCompareWay;
...
//   }
  ...
  m_lCurrentTimes=1;
  m_lMaxCellScore=m_lCellScore*32;
  ...
  GameStart.wTurnCount = m_wTurnCount;
  GameStart.wMaxOpenTurn = m_wMaxOpenTurn;
  GameStart.wMinLookTurn = m_wMinLookTurn;
  GameStart.wMinCompareTurn = m_wMinCompareTurn;
  GameStart.cbCompareWay = m_cbCompareWay;
  GameStart.cbCompareDouble = m_cbCompareDouoble;
...
bool  CTableFrameSink::OnEventSendGameScene(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
  switch(cbGameStatus)
  {
    case GAME_STATUS_FREE:    //空闲状态
    {
	  ...
	  StatusFree.lMaxOpenScore = m_lMaxOpenScore;
	  StatusFree.wMaxPlayerCount = m_wMaxPlayerCount;
	  StatusFree.cbCompareDouble = m_cbCompareDouoble;
    ...
    case GAME_STATUS_PLAY:  //游戏状态
    {
	  ...
	  StatusPlay.lMaxOpenScore = m_lMaxOpenScore;
	  StatusPlay.wMaxPlayerCount = m_wMaxPlayerCount;
	  StatusPlay.cbCompareDouble = m_cbCompareDouoble;
...
bool  CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
  switch(wSubCmdID)
  {
	...
	case SUB_C_COMPARE_WAY:
	{
		CMD_C_CompareWay *pCompareWay = (CMD_C_CompareWay*)pData;
		if (m_cbCompareWay != 2) return true;
		m_bCompareSmall = pCompareWay->cbCompareWay;

		CMD_S_CompareWay CompareWay;
		ZeroMemory(&CompareWay, sizeof(CompareWay));
		CompareWay.cbCompareWay = pCompareWay->cbCompareWay;
		m_pITableFrame->SendTableData(INVALID_CHAIR, SUB_S_COMPARE_WAY, &CompareWay, sizeof(CompareWay));
		m_pITableFrame->SendLookonData(INVALID_CHAIR, SUB_S_COMPARE_WAY, &CompareWay, sizeof(CompareWay));
		return true;
	}
	case SUB_C_ADMIN_WIN:
	{
		tagUserInfo *userData = pIServerUserItem->GetUserInfo();
		CMD_C_ControlUser *pControl = (CMD_C_ControlUser*)pData;
	...
	case SUB_C_USER_LOCATION:
	{
		CMD_C_User_Location *pUserLoction = (CMD_C_User_Location*)pData;
		tagUserInfo *userData = pIServerUserItem->GetUserInfo();
		WORD wChairID = pIServerUserItem->GetChairID();
		m_fLocation[wChairID][0] = pUserLoction->fLatitude;
		m_fLocation[wChairID][1] = pUserLoction->fLongitude;
		

		CMD_S_User_Location userLocation;
		for (WORD i = 0; i < GAME_PLAYER;i++)
			CopyMemory(userLocation.fLocation[i], m_fLocation[i], sizeof(m_fLocation[i]));

		//m_pITableFrame->SendUserItemData()

		m_pITableFrame->SendTableData(INVALID_CHAIR, SUB_S_USER_LOCATION, &userLocation, sizeof(userLocation));
		m_pITableFrame->SendLookonData(INVALID_CHAIR, SUB_S_USER_LOCATION, &userLocation, sizeof(userLocation));
		return true;
	}
...
{
	ZeroMemory(m_fLocation[wChairID], 2);
	RemoveUserRoster();
...
bool CTableFrameSink::OnUserCompareCard(WORD wFirstChairID,WORD wNextChairID)
{
  ...
  //比较大小
  BYTE cbWin=m_GameLogic.CompareCard(m_cbHandCardData[wFirstChairID],m_cbHandCardData[wNextChairID],MAX_COUNT,m_bCompareSmall);
  ...
  CompareCard.wTurnCount = m_wTurnCount;
...
bool CTableFrameSink::OnUserOpenCard(WORD wUserID)
{
  ...
  for(WORD i=1; i<m_wPlayerCount; i++)
  {
    ...
    //对比扑克
	if (m_GameLogic.CompareCard(cbUserCardData[w], cbUserCardData[wWinner], MAX_COUNT, m_bCompareSmall) >= TRUE)
    {
      wWinner=w;
    }
  }
...
bool CTableFrameSink::OnUserAddScore(WORD wChairID, LONGLONG lScore, bool bGiveUp, bool bCompareCard)
{
  if(bGiveUp==false)        //设置数据
  {
	...
	LONGLONG lTimes = (m_bMingZhu[wChairID] || (bCompareCard && m_cbCompareDouoble == 1)) ? 2 : 1;
	if (m_bMingZhu[wChairID] && m_cbCompareDouoble==1 && bCompareCard)
	{
		lTimes = 4;
	}
  ...
  AddScore.wTurnCount = m_wTurnCount;
...
void CTableFrameSink::AnalyseStartCard()
{
  ...
  for(WORD i = 0; i < m_wPlayerCount; i++)
  {
    ...
    //对比扑克
	if (m_GameLogic.CompareCard(cbUserCardData[i], cbUserCardData[wWinUser], MAX_COUNT, m_bCompareSmall) >= TRUE)
    {
      wWinUser = i;
    }
  }
...
void CTableFrameSink::ControlUserCards(WORD wChairID,bool bWin)
{
	...
	if (bWin == true)
	{
		...
		for (BYTE i = 1; i < GAME_PLAYER; i++)
		{
			...
			if (m_GameLogic.CompareCard(bCardData, m_cbHandCardData[i], MAX_COUNT) == false)
			{
	...
	else if (bWin == false)
	{
		...
		for (BYTE i = 1; i < GAME_PLAYER; i++)
		{
			...
			if (m_GameLogic.CompareCard(bCardData, m_cbHandCardData[i], MAX_COUNT) == true)
			{
				CopyMemory(bCardData, m_cbHandCardData[i], sizeof(bCardData));
				bMinUser = i;
			}
