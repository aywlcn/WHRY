...
#define SUB_S_COMPARE_WAY		118					//比牌方式
...
#define	SUB_S_USER_LOCATION			119					//玩家位置信息

#define	SUB_S_CONTROL_RESULT		120


struct PriviteRoomRule
{
	WORD wMinLookTurn;
	WORD wMinCompareTurn;
	WORD wMaxOpenTurn;
	BYTE cbCompareDouoble;
	LONGLONG wMaxPlayer;
	LONGLONG lCellScore;
	LONGLONG lMaxOpenScore;
};
...
struct CMD_S_StatusFree
{
  ...
  WORD					wMaxOpenTurn;			//封顶轮数
  WORD					wMinCompareTurn;		//最小比牌轮数
  WORD					wMinLookTurn;			//最小看牌论事
  WORD					wMaxPlayerCount;		//玩家人数
  LONGLONG				lMaxOpenScore;			//封顶分数
  BYTE					cbCompareDouble;		//比牌双倍
};
...
struct CMD_S_GameStart
{
  ...
  WORD				wTurnCount;					//下注轮数
  WORD				wMinLookTurn;				//必闷轮数
  WORD				wMinCompareTurn;			//比牌轮数
  WORD				wMaxOpenTurn;				//封顶轮数
...
struct CMD_S_CompareWay
{
	BYTE			 cbCompareWay;			 //比大比小
};


struct CMD_S_User_Location
{
	FLOAT	fLocation[GAME_PLAYER][2];
};
...
#define SUB_C_COMPARE_WAY		9					//比牌方式
...
#define SUB_C_USER_LOCATION     15					//玩家位置信息
...
struct CMD_C_CompareWay
{
	BYTE			 cbCompareWay;			 //0 比大  1 比小
};


struct CMD_C_User_Location
{
	FLOAT			fLatitude;
	FLOAT			fLongitude;
};
