...
BYTE CGameLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	BYTE bKingCount = 0, bTenCount = 0, bHuaCount = 0;
	for (BYTE i = 0; i<cbCardCount; i++)
	{
		BYTE bTmpVal = GetCardValue(cbCardData[i]);
		if (bTmpVal > 10 && bTmpVal < 14)
		{
			bHuaCount++;
		}
		else if (bTmpVal == 10)
	...
	if (bHuaCount == MAX_COUNT || (bHuaCount + bKingCount == MAX_COUNT)) 
		return OX_FIVEKING;
	else if ((bHuaCount == MAX_COUNT - 1 || bHuaCount+bKingCount == MAX_COUNT - 1) && bTenCount == 1)
		return OX_FOURKING;


	if (bKingCount == 2)
		return 10;
	else if (bKingCount == 1)
	{
		BYTE bTemp[MAX_COUNT];
		int bSum = 0;
		int iIdx = 0;
		...
		{
			if (cbCardData[i] == 0x4E || cbCardData[i] == 0x4F) continue;
			bTemp[iIdx] = GetCardLogicValue(cbCardData[i]);
			bSum += bTemp[iIdx];
			iIdx++;
		}
		/*
		for (BYTE i = 0; i < 4; i++)
		{
			if ((bSum - bTemp[i]) % 10 == 0)   //任意三张组成10则为牛牛
				return 10;
		}

		for (BYTE i = 0; i < 3; i++)  //任意两张组成10则为牛牛
		{
		...
		*/
		...
		BYTE bNew[MAX_COUNT] = {0};
		WORD wNewSum = bSum;
		memcpy(bNew,bTemp,MAX_COUNT);

		BYTE bTempMax = 0;
		for (int m = 1; m < 11; m++)//王牌当万能牌替换，找出最大牛
		{
			bNew[4] = m;
			wNewSum += m;
			for (BYTE i = 0; i < 4; i++)   
			{
				...
				{
					if ((wNewSum - bNew[i] - bNew[j]) % 10 == 0)
					{
						BYTE bNiuVal = ((bNew[i] + bNew[j]) > 10) ? (bNew[i] + bNew[j] - 10) : (bNew[i] + bNew[j]);
						if (bNiuVal > bTempMax)
						{
							bTempMax = bNiuVal;
						}
...
void CGameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount,bool bHaveKing)
{
	...
	int cbCardCount = CountArray(m_cbCardListData);
	if (!bHaveKing)
		cbCardCount = cbCardCount - 2;

	BYTE cbCardData[CountArray(m_cbCardListData)];
	CopyMemory(cbCardData, m_cbCardListData, cbCardCount);

	//混乱扑克
	BYTE bRandCount=0,bPosition=0;
	do
	{
		bPosition = rand() % (cbCardCount - bRandCount);
		cbCardBuffer[bRandCount++]=cbCardData[bPosition];
		cbCardData[bPosition] = cbCardData[cbCardCount - bRandCount];
...
bool CGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount,BOOL FirstOX,BOOL NextOX)
{
	...
	*/
	...
	//比较是否有王，相同牛数，有王的比没有王的小
	bool bFlagFirst = false, bFlagNext = false;
	...
	if (bFlagFirst == true && bFlagNext == false) return false;
	else if (bFlagFirst == false && bFlagNext == true) return true;
