...
bool CDlgCustomRule::FillDataToControl()
{
	...
	//游戏玩法
	((CButton *)GetDlgItem(IDC_RADIO1))->SetCheck(m_CustomRule.cbSendCardType == 32);
	((CButton *)GetDlgItem(IDC_RADIO2))->SetCheck(m_CustomRule.cbSendCardType == 33);

	//大小王
	((CButton *)GetDlgItem(IDC_CHECK_KING))->SetCheck(m_CustomRule.cbHasKing == 42);
	return true;
}
...
bool CDlgCustomRule::FillControlToData()
{
	...
	if (BST_CHECKED == IsDlgButtonChecked(IDC_CHECK_KING))
	{
		m_CustomRule.cbHasKing = 42;
	}
	else
	{
		m_CustomRule.cbHasKing = 43;
	}

	if (TRUE == IsDlgButtonChecked(IDC_RADIO1))
	{
		m_CustomRule.cbSendCardType = 32;
	}
	else if (TRUE == IsDlgButtonChecked(IDC_RADIO2))
	{
		m_CustomRule.cbSendCardType = 33;
	}
	else
	{
		m_CustomRule.cbSendCardType = 32;
