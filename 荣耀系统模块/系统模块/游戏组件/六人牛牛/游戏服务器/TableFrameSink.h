...
protected:
	BYTE							m_cbKingConfig;
	BYTE							m_cbBankerConfig;
	BYTE							m_cbCardConfig;

	WORD							m_wCheatChairID;			//作弊玩家
	BOOL							m_cbCheatWin;				//控制输赢
...
protected:
	...
	LONGLONG						m_lBonus;
	tagPersonalSpecilInfo			m_pSpecilInfo[GAME_PLAYER];
...
protected:
	...
	BYTE							m_cbCallBankerMul[GAME_PLAYER];			//叫庄倍数
	BYTE							m_cbOpenStatus[GAME_PLAYER];			//开牌状态
...
protected:
	//叫庄事件
	bool OnUserCallBanker(WORD wChairID, BYTE bBankerTimes);
	//加注事件
...
protected:
	...
	void ControlUserCards(WORD wChairID, bool bWin = true);
};
