...
BYTE						g_cbKingCfg = GT_NOKING;					//无大小王
BYTE						g_cbSendCardType = ST_SENDFOUR;				//发牌方式
...
CTableFrameSink::CTableFrameSink()
{
	...
	m_cbKingConfig = GT_NOKING;
	m_cbCardConfig = ST_SENDFOUR;
	...
	ZeroMemory(m_cbOpenStatus, sizeof(m_cbOpenStatus));
	ZeroMemory(m_pSpecilInfo, sizeof(m_pSpecilInfo));
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	m_cbKingConfig = pGameCustomRule->cbHasKing;
	g_cbSendCardType = pGameCustomRule->cbSendCardType;
...
bool CTableFrameSink::OnEventGameStart()
{
	...
	GameStart.wBankerUser = INVALID_CHAIR;
	CopyMemory(GameStart.cbPlayerStatus, m_cbPlayStatus, sizeof(m_cbPlayStatus));

	BYTE bTempArray[GAME_PLAYER*MAX_COUNT] = {0};
	m_GameLogic.RandCardList(bTempArray, sizeof(bTempArray), m_cbKingConfig == 42);
	for (WORD i = 0; i<m_wPlayerCount; i++)
		CopyMemory(m_cbHandCardData[i], &bTempArray[i*MAX_COUNT], MAX_COUNT);
	if (m_wCheatChairID != INVALID_CHAIR)
		ControlUserCards(m_wCheatChairID, m_cbCheatWin == TRUE);
	m_wCheatChairID = INVALID_CHAIR;
	for (WORD i=0;i<m_wPlayerCount;i++)
	{
		IServerUserItem *pIServerUser=m_pITableFrame->GetTableUserItem(i);	
		...
		//发四张
		CopyMemory(GameStart.cbCardData[i], &m_cbHandCardData[i], MAX_COUNT - 1);
	}
	GameStart.bgtConfig = BGT_ROB;
	GameStart.stConfig = (INT)g_cbSendCardType;
	GameStart.btConfig = m_cbKingConfig;
	...
	case GER_NORMAL:		//常规结束
		{
			...
			CopyMemory(GameEnd.cbCardData, m_cbHandCardData, sizeof(cbUserCardData));

			for (WORD i = 0; i < GAME_PLAYER;i++)
			{
				if (m_cbPlayStatus[i] == FALSE)continue;
				BYTE cbOx = m_GameLogic.GetCardType(GameEnd.cbCardData[i], MAX_COUNT);
				m_GameLogic.GetOxCard(GameEnd.cbCardData[i], MAX_COUNT);
				GameEnd.cbCardType[i] = cbOx;
			}
 			...
 			for (WORD i = 0; i<m_wPlayerCount; i++)
			{
				...
				if (m_GameLogic.CompareCard(cbUserCardData[i], cbUserCardData[m_wBankerUser], MAX_COUNT, GameEnd.cbCardType[i], GameEnd.cbCardType[m_wBankerUser]))
				{
					int winTime = m_GameLogic.GetTimes(cbUserCardData[i], MAX_COUNT);
					GameEnd.lGameScore[i] = m_lTableScore[i] * winTime * m_cbCallBankerMul[m_wBankerUser];
					GameEnd.lGameScore[m_wBankerUser]-=GameEnd.lGameScore[i];
				}
				else
				{
					int winTime = m_GameLogic.GetTimes(cbUserCardData[m_wBankerUser], MAX_COUNT);
					GameEnd.lGameScore[i] = (-1)*m_lTableScore[i] * winTime * m_cbCallBankerMul[m_wBankerUser];
					GameEnd.lGameScore[m_wBankerUser] -= GameEnd.lGameScore[i];
				}
			}
			for (WORD i = 0; i < m_wPlayerCount; i++)
			{
				if (GameEnd.lGameScore[i] >= 0)
					m_pSpecilInfo[i].wWinCount++;
				else
					m_pSpecilInfo[i].wLoseCount++;
			}
...
// 				{
// 					GameEnd.lGameScore[i] = m_lTableScore[i] * wWinTimes[m_wBankerUser] * m_cbCallBankerMul[m_wBankerUser] * m_lTableScore[i];
// 					GameEnd.lGameScore[m_wBankerUser]-=GameEnd.lGameScore[i];
// 					m_lTableScore[i]=0;
// 
...
// 				else					//庄家胜利
// 				{
// 					GameEnd.lGameScore[i] = (-1)*m_lTableScore[i] * wWinTimes[m_wBankerUser] * m_cbCallBankerMul[m_wBankerUser] * m_lTableScore[i];
...
// 			}
			m_pITableFrame->GetSpeicalInfo((byte*)m_pSpecilInfo, sizeof(m_pSpecilInfo));
	...
	case GAME_STATUS_FREE:		//空闲状态
		{
			...
			StatusFree.stConfig = (INT)g_cbSendCardType;
			StatusFree.btConfig = m_cbKingConfig;
	...
	case GS_TK_CALL:	//叫庄状态
		{
			...
			CopyMemory(StatusCall.cbCallBankerStatus, m_cbCallStatus, sizeof(StatusCall.cbCallBankerStatus));
			CopyMemory(StatusCall.cbCallBankerTimes, m_cbCallBankerMul, sizeof(StatusCall.cbCallBankerTimes));
			...
			for (WORD i = 0; i < GAME_PLAYER;i++)
			{
				ZeroMemory(StatusCall.cbCardData[i], MAX_COUNT);
				CopyMemory(StatusCall.cbCardData[i], m_cbHandCardData[i], MAX_COUNT - 1);
			}
			...
			RobotConfig.lRobotBankGetBanker = pCustomRule->lRobotBankGetBanker;
			RobotConfig.lRobotBankStoMul = pCustomRule->lRobotBankStoMul;
			StatusCall.stConfig = (INT)g_cbSendCardType;
			StatusCall.btConfig = m_cbKingConfig;
	...
	case GS_TK_SCORE:	//下注状态
		{
			...
			for (WORD i = 0; i < GAME_PLAYER; i++)
			{
				ZeroMemory(StatusScore.cbCardData[i], MAX_COUNT);
				CopyMemory(StatusScore.cbCardData[i], m_cbHandCardData[i], MAX_COUNT - 1);
			}
			//CopyMemory(StatusScore.cbCardData, m_cbHandCardData, sizeof(StatusScore.cbCardData));
			...
			StatusScore.stConfig = (INT)g_cbSendCardType;
			StatusScore.btConfig = m_cbKingConfig;
			//权限判断
			if(CUserRight::IsGameCheatUser(pIServerUserItem->GetUserRight()))
			{
				CMD_S_ADMIN_STORAGE_INFO StorageInfo;
	...
	case GS_TK_PLAYING:	//游戏状态
		{
			...
			CopyMemory(StatusPlay.cbOpenStatus, m_cbOpenStatus, sizeof(StatusPlay.cbOpenStatus));
			...
			StatusPlay.stConfig = (INT)g_cbSendCardType;
			StatusPlay.btConfig = m_cbKingConfig;
			...
			case GS_TK_CALL:			//用户叫庄
				{
					for (WORD i = 0; i < GAME_PLAYER;i++)
					{
						if (m_cbPlayStatus[i] == TRUE && m_cbCallStatus[i] == FALSE)
						{
...
bool CTableFrameSink::OnGameMessage(WORD wSubCmdID, void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_C_CALL_BANKER:			//用户叫庄
		{
			...
			//消息处理
			bResult=OnUserCallBanker(pUserData->wChairID,pCallBanker->cbBankerTimes);
			break;
		}
	...
	case SUB_C_ADMIN_WIN:
		{
			tagUserInfo *userData = pIServerUserItem->GetUserInfo();
			CMD_C_ControlUser *pControl = (CMD_C_ControlUser*)pDataBuffer;
			CMD_S_Control_Result ControlResult;
			if (CUserRight::IsGameCheatUser(pIServerUserItem->GetUserRight()) == false)
			{
				ControlResult.cbResult = 0;
			}
			else
			{
				ControlResult.cbResult = 1;
				m_wCheatChairID = pControl->wChairID;
				m_cbCheatWin = pControl->cbWin;
			}
			ControlResult.wChairID = pControl->wChairID;
			ControlResult.cbWin = pControl->cbWin;
			m_pITableFrame->SendTableData(pIServerUserItem->GetChairID(), SUB_S_CONTROL_RESULT, &ControlResult, sizeof(ControlResult));
...
//叫庄事件
bool CTableFrameSink::OnUserCallBanker(WORD wChairID, BYTE bBankerTimes)
{
	//状态效验
	int gameStata = m_pITableFrame->GetGameStatus();
	ASSERT(m_pITableFrame->GetGameStatus()==GS_TK_CALL);
	if (m_pITableFrame->GetGameStatus() != GS_TK_CALL && m_cbCallStatus[wChairID] == FALSE) return true;

	//设置变量
	m_cbCallStatus[wChairID] = TRUE;
	m_cbCallBankerMul[wChairID] = bBankerTimes;

	CMD_S_CallBanker CallBanker;
	CallBanker.wCallBanker = wChairID;
	CallBanker.bFirstTimes = false;
	CallBanker.cbBankerTimes = bBankerTimes;
	...
	//找到最大抢庄倍数
	BYTE cbMaxBankTimes = 0;
	for (WORD i = 0; i < GAME_PLAYER;i++)
	{
		if (m_cbCallStatus[i] == TRUE && m_cbCallBankerMul[i] > cbMaxBankTimes)
			cbMaxBankTimes = m_cbCallBankerMul[i];
	}

	//确定庄家
	while (true)
	{
		WORD wBanker = rand() % GAME_PLAYER;
		if (m_cbCallStatus[wBanker] == TRUE && m_cbCallBankerMul[wBanker] == cbMaxBankTimes)
		{
	...
	CMD_S_CallBankerInfo CallBankerInfo;

	CopyMemory(CallBankerInfo.cbCallBankerStatus, m_cbCallStatus, sizeof(m_cbCallStatus));
	CopyMemory(CallBankerInfo.cbCallBankerTimes, m_cbCallBankerMul, sizeof(m_cbCallBankerMul));
	CallBankerInfo.wBankerUser = m_wBankerUser;
	
	m_pITableFrame->SendTableData(INVALID_CHAIR, SUB_S_CALL_BANKERINFO, &CallBankerInfo, sizeof(CallBankerInfo));
	m_pITableFrame->SendLookonData(INVALID_CHAIR, SUB_S_CALL_BANKERINFO, &CallBankerInfo, sizeof(CallBankerInfo));
...
// 	}
	...
	if(bUserCount==m_wPlayerCount)
	{
		...
		SendCard.wBankerUser = m_wBankerUser;
...
//		m_pITableFrame->SendTableData(INVALID_CHAIR, SUB_S_ALL_ADDSCORE, NULL, 0);
		//构造数据
// 		CMD_S_SendCard SendCard;
// 		ZeroMemory(SendCard.cbCardData,sizeof(SendCard.cbCardData));
// 
...
bool CTableFrameSink::OnUserOpenCard(WORD wChairID, BYTE bOx)
{
	...
	CopyMemory(OpenCard.cbCardData, m_cbHandCardData[wChairID], MAX_COUNT);
	OpenCard.cbCardType = m_GameLogic.GetCardType(OpenCard.cbCardData, MAX_COUNT);
	m_GameLogic.GetOxCard(OpenCard.cbCardData, MAX_COUNT);
...
void CTableFrameSink::ControlUserCards(WORD wChairID, bool bWin)
{
	//控制玩家赢
	...
	if (bWin == true)
	{
		...
		CopyMemory(bCardData, m_cbHandCardData[0], sizeof(bCardData));
		BOOL firstOx = m_GameLogic.GetCardType(bCardData, MAX_COUNT)>0?TRUE:FALSE;
		for (BYTE i = 1; i < GAME_PLAYER; i++)
		{
			...
			BOOL nextOx = m_GameLogic.GetCardType(bCardData, MAX_COUNT)>0?TRUE:FALSE;
			if (m_GameLogic.CompareCard(bCardData, m_cbHandCardData[i], MAX_COUNT, firstOx, nextOx) == false)
			{
				CopyMemory(bCardData, m_cbHandCardData[i], sizeof(bCardData));
				bMaxUser = i;
			}
		}
		if (bMaxUser == wChairID)
			return;
		CopyMemory(m_cbHandCardData[bMaxUser], m_cbHandCardData[wChairID], MAX_COUNT);
		CopyMemory(m_cbHandCardData[wChairID], bCardData, MAX_COUNT);
	}
	//控制玩家输
	else if (bWin == false)
	{
		...
		for (BYTE i = 1; i < GAME_PLAYER; i++)
		{
			...
			BOOL nextOx = m_GameLogic.GetCardType(bCardData, MAX_COUNT)>0?TRUE:FALSE;
			if (m_GameLogic.CompareCard(bCardData, m_cbHandCardData[i], MAX_COUNT, firstOx, nextOx) == true)
			{
				CopyMemory(bCardData, m_cbHandCardData[i], sizeof(bCardData));
				bMinUser = i;
			}
		}
		if (bMinUser == wChairID)
			return;
		CopyMemory(m_cbHandCardData[bMinUser], m_cbHandCardData[wChairID], MAX_COUNT);
		CopyMemory(m_cbHandCardData[wChairID], bCardData, MAX_COUNT);
	}
}
