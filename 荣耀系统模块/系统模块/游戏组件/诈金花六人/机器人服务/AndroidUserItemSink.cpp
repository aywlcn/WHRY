  ...
  switch(nTimerID)
  {
    ...
    case IDI_USER_ADD_SCORE:  //加注定时器
    {
      ...
      if(m_lTableScore[wMeChairID] == m_lCellScore)
      {
        ...
        //看牌概率
        if(m_bMingZhu == false && m_wTurnCount >= m_wMinLookTurn)
        {
        ...
        //庄家比牌
        if(m_wBankerUser == wMeChairID && m_bMingZhu == true && m_wTurnCount >= m_wMinCompareTurn)
        {
...
bool CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pData, WORD wDataSize)
{
  switch(cbGameStatus)
  {
    ...
    case GAME_STATUS_PLAY:    //游戏状态
    {
	  ...
	  m_wTurnCount = pStatusPlay->wTurnCount;
	  m_wMinLookTurn = pStatusPlay->wMinLookTurn;
	  m_wMinCompareTurn = pStatusPlay->wMinCompareTurn;
...
bool CAndroidUserItemSink::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
  ...
  m_wMaxOpenTurn = pGameStart->wMaxOpenTurn;
  m_wMinLookTurn = pGameStart->wMinLookTurn;
  m_wMinCompareTurn = pGameStart->wMinCompareTurn;
...
bool CAndroidUserItemSink::OnSubAddScore(const void * pBuffer, WORD wDataSize)
{
  ...
  m_wTurnCount = pAddScore->wTurnCount;
  WORD wMeChairID = m_pIAndroidUserItem->GetChairID();
...
bool CAndroidUserItemSink::OnSubCompareCard(const void * pBuffer, WORD wDataSize)
{
  ...
  m_wTurnCount = pCompareCard->wTurnCount;
