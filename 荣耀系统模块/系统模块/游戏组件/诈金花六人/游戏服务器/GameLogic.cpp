...
//对比扑克
BYTE CGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount, BOOL bCompareSamll)
{
  //设置变量
  ...
  {
	  if (bCompareSamll)
		 return (cbFirstType>cbNextType) ? 0 : 1;
	  else
  ...
  switch(cbFirstType)
  {
    ...
    case CT_JIN_HUA:    //金花
    {
      ...
      for(BYTE i=0; i<cbCardCount; i++)
      {
        ...
        {
		  if (bCompareSamll)
			  return (cbFirstValue>cbNextValue) ? 0 : 1;
		  else
    ...
    case CT_DOUBLE:     //对子
    {
      ...
      {
		if (bCompareSamll)
			return (bFirstDouble > bNextDouble) ? 0 : 1;
		else
      ...
      {
		if (bCompareSamll)
			return (bFirstSingle>bNextSingle) ? 0 : 1;
		else
