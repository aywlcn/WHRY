...
CTableFrameSink::CTableFrameSink()
{
	...
	m_wCheatArea = 255;
	m_hInst = LoadLibrary(TEXT("SgTbBattleServerControl.dll"));
	if ( m_hInst )
	{
		typedef void * (*CREATE)(); 
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf(m_szConfigFileName,sizeof(m_szConfigFileName),TEXT("%s\\SgTbBattleConfig.ini"),szPath);
	...
	case GAME_SCENE_GAME_END:			//结束状态
		{
			...
			StatusPlay.cbTimeLeave = 10;//(BYTE)(nTotalTime-__min(dwPassTime,(DWORD)nTotalTime));
...
//#endif
	...
	//设置数目
	for (int i=0; i<CountArray(m_cbCardCount); ++i) m_cbCardCount[i]=3;
...
LONGLONG CTableFrameSink::CalculateScore()
{
	...
	int CompareResult[AREA_COUNT] = { 0 };
	DeduceWinnerEx(CompareResult);

	int sortIndex[AREA_COUNT] = { 0, 1, 2, 3 };
	for (int i = 0; i < AREA_COUNT; i++)
	{
		for (int j = i+1; j < AREA_COUNT; j++)
		{
			if (CompareResult[i] < CompareResult[j])
			{
				int temp = sortIndex[i];
				int temp1 = CompareResult[i];
				CompareResult[i] = CompareResult[j];
				CompareResult[j] = temp1;
	...
	SCORE lScoreResult[AREA_COUNT] = { 0 };
	SCORE lAllLostScore = 0;
	for (int i = 0; i < AREA_COUNT; i++)
		lAllLostScore += m_lAllJettonScore[i];
	for (int i = 0;i<AREA_COUNT;i++)
	{
		int _index = sortIndex[i];
		lScoreResult[_index] += __min(m_lAllJettonScore[_index]*2,lAllLostScore);
		lAllLostScore -= lScoreResult[_index];
	}
	...
	for (WORD wChairID = 0; wChairID < GAME_PLAYER; wChairID++)
	{
		...
		{
			if (m_lAllJettonScore[wAreaIndex] == 0) continue;
			if (m_lUserJettonScore[wAreaIndex][wChairID] > 0)
			{
				if (lScoreResult[wAreaIndex] == m_lAllJettonScore[wAreaIndex]*2)
				{
					m_lUserWinScore[wChairID] += m_lUserJettonScore[wAreaIndex][wChairID];
					m_lUserReturnScore[wChairID] += m_lUserJettonScore[wAreaIndex][wChairID];
				}
				else if (lScoreResult[wAreaIndex] == 0)
				{
					m_lUserWinScore[wChairID] -= m_lUserJettonScore[wAreaIndex][wChairID];
				...
				{
					SCORE lUserRetScore = lScoreResult[wAreaIndex] * m_lUserJettonScore[wAreaIndex][wChairID] / m_lAllJettonScore[wAreaIndex];
					m_lUserWinScore[wChairID] += (lUserRetScore - m_lUserJettonScore[wAreaIndex][wChairID]);
	...
	GameRecord.bWinShunMen= (lScoreResult[0] == m_lAllJettonScore[0]*2);
	GameRecord.bWinDuiMen = (lScoreResult[1] == m_lAllJettonScore[1] * 2);
	GameRecord.bWinDaoMen = (lScoreResult[2] == m_lAllJettonScore[2] * 2);
	GameRecord.bWinHuang = (lScoreResult[3] == m_lAllJettonScore[3] * 2);
...
{
	if (m_wCheatArea == 255 || m_wCheatArea < 0 || m_wCheatArea > 3) return;
	int compareResult[4] = { 0 };
	DeduceWinnerEx(compareResult);
	int minIndex = 0;
	int minValue = 255;
	for (int i = 0; i < 4;i++)
...
void CTableFrameSink::DeduceWinnerEx(int *compareResult)
{
	for (WORD i = 0; i < AREA_COUNT; i++)
	{
		for (WORD j = i + 1; j < AREA_COUNT; j++)
		{
			int ret = m_GameLogic.CompareCard(m_cbTableCardArray[i], 3, m_cbTableCardArray[j], 3);
			compareResult[i] += ret;
			compareResult[j] -= ret;
		}
	}
...
// 判断系统得分
void CTableFrameSink::JudgeSystemScore(BYTE bCardData[4][3], LONGLONG& lSystemScore, bool& bAllEat )
{
	// 系统输赢
	bAllEat = true;
	lSystemScore = 0l;
