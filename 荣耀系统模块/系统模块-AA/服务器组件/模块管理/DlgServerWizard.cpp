...
BOOL CDlgServerOptionItem5::OnInitDialog()
{
	...
	((CButton *)GetDlgItem(IDC_RADIO_PAY_FANGZHU))->SetCheck(m_pPersonalRoomOption->cbPayType);
	...
	{
		SetDlgItemInt(IDC_EDIT_TURN_NUM + i * 4, m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPlayTurnLimit);
		SetDlgItemInt(IDC_EDIT_CREATE_FEE + i * 4, m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayFee);
		SetDlgItemInt(IDC_EDIT_CREATE_FEE6 + i * 4, m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayAA);
		SetDlgItemInt(IDC_EDIT_ROOM_INISCORE + i * 4, m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwIniScore);
		if (i == 0)
		{
			if (m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPlayTurnLimit == 0 || m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwIniScore == 0 
				|| m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayFee == 0 || m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayAA == 0)
			{
				SetDlgItemInt(IDC_EDIT_TURN_NUM + i * 4, 5);
				SetDlgItemInt(IDC_EDIT_CREATE_FEE + i * 4, 1);
				SetDlgItemInt(IDC_EDIT_CREATE_FEE6 + i * 4, 1);
				SetDlgItemInt(IDC_EDIT_ROOM_INISCORE + i * 4, 1000);
			}
		}
	}
...
bool CDlgServerOptionItem5::SaveInputInfo()
{
	...
	//需要结合cbCardOrBean变量使用，如果为1房卡模式，则生效，否则不生效
	//如果房卡模式，不选择，默认为房主支付
	//0 表示 房主支付  1 表示 AA支付
	if (((CButton *)GetDlgItem(IDC_RADIO_PAY_AA))->GetCheck() == BST_CHECKED)
	{
		m_pPersonalRoomOption->cbPayType = 1;
	}
	else 
	{
		m_pPersonalRoomOption->cbPayType = 0;
	}
	...
	for (int i = 0; i < FEE_OPTION_COUNT; i++)
	{
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPlayTurnLimit = GetDlgItemInt(IDC_EDIT_TURN_NUM + i * 4);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayFee = GetDlgItemInt(IDC_EDIT_CREATE_FEE + i * 4);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayAA = GetDlgItemInt(IDC_EDIT_CREATE_FEE6 + i * 4);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwIniScore = GetDlgItemInt(IDC_EDIT_ROOM_INISCORE + i * 4);
	}
