...
public:
	...
	virtual bool SetGameRule(byte *cbRule, int len) = NULL;
...
public:
	...
	//取消创建
	virtual VOID CancelCreateTable(DWORD dwUserID, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID, TCHAR * szRoomID, BYTE cbPayType) = NULL;
	//开始游戏写入参与信息
	virtual VOID PersonalRoomWriteJoinInfo(DWORD dwUserID, WORD wTableID, TCHAR * szRoomID)=NULL;
	//扣除AA房费（除房主外的其他玩家）
	virtual VOID PayAATableFee(DWORD dwUserID, WORD wTableID, TCHAR * szRoomID, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwClientAddr) = NULL;
};
