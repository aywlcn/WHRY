...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_WRITE_JOIN_INFO:
		...
		break;
	case DBR_GR_PAY_AA_TABLE_FEE:
		{
			bSucceed = OnRequestPayAATableFee(dwContextID, pData, wDataSize, dwUserID);
		}
		break;
	}
...
bool CDataBaseEngineSink::OnRequestCreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	try
	{
		...
		//m_TreasureDBAide.AddParameter(TEXT("@cbPayType"), pCreateTable->cbPayType);
		...
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GR_CreateTableFee"),true)==DB_SUCCESS)
		{
			...
			CreateSuccess.cbPayType = pCreateTable->cbPayType;

			TCHAR szInfo[260] = {0};
			wsprintf(szInfo, TEXT("ptdt *** 剩余房卡数 = %d"), CreateSuccess.lRoomCard  );
...
bool CDataBaseEngineSink::OnRequsetInsertCreateRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		...
		m_TreasureDBAide.AddParameter(TEXT("@cbPayType"), pCreateRecord->cbPayType);
...
bool CDataBaseEngineSink::OnRequestCancelCreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		...
		if (pCancelCreateTable->dwReason == CANCELTABLE_REASON_ENFOCE || CANCELTABLE_REASON_NOT_START == pCancelCreateTable->dwReason )
		{
			...
			m_TreasureDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
			m_TreasureDBAide.AddParameter(TEXT("@cbPayType"), pCancelCreateTable->cbPayType);
...
bool CDataBaseEngineSink::OnRequestLoadPersonalParameter(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		...
		if(lResultCode == DB_SUCCESS)
		{
			...
			pPersonalRoomOption.cbPayType = m_PlatformDBAide.GetValue_BYTE(TEXT("PayOwnerOrAA"));
...
//扣除AA房费
bool CDataBaseEngineSink::OnRequestPayAATableFee(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		DBR_GR_PayAATableInfo * pPayAAInfo = (DBR_GR_PayAATableInfo *)pData;
		//效验参数
		ASSERT(wDataSize == sizeof(DBR_GR_PayAATableInfo));
		if (wDataSize != sizeof(DBR_GR_PayAATableInfo)) return false;
		...
		//用户信息
		m_PlatformDBAide.AddParameter(TEXT("@dwUserID"), pPayAAInfo->dwUserID);
		m_PlatformDBAide.AddParameter(TEXT("@dwServerID"), pPayAAInfo->szRoomID);
		m_PlatformDBAide.AddParameter(TEXT("@dwDrawCountLimit"), pPayAAInfo->dwDrawCountLimit);
		m_PlatformDBAide.AddParameter(TEXT("@dwDrawTimeLimit"), pPayAAInfo->dwDrawTimeLimit);

		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE*)&pPayAAInfo->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		m_PlatformDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);


		OutputDebugStringA("ptdt ***** 执行扣AA房卡  存储过程");
		//执行查询
		LONG lResultCode = m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GR_AATableFeePay"), false);
	}
	catch (...)
	{
		TCHAR szInfo[260] = { 0 };
		wsprintf(szInfo, TEXT("OnRequestPayAATableFee 函数中有错误"));
		CTraceService::TraceString(szInfo, TraceLevel_Exception);
		return false;
	}
