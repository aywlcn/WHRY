		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if (pIServerUserItem!=NULL)
			{
				...
				if(m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL &&  m_szPersonalTableID != NULL )		
				{
					...
					//AA模式扣除除房主以外的人应付房卡，第一局扣，后面不扣
// 					if (pIServerUserItem->GetUserID() != m_dwTableOwnerID && m_dwPersonalPlayCount == 1)
// 					{
// 						m_pIMainServiceFrame->PayAATableFee(pIServerUserItem->GetUserID(), m_wTableID, m_szPersonalTableID, 
// 												m_dwDrawCountLimit, m_dwDrawTimeLimit, pIServerUserItem->GetClientAddr());
// 					}
				}
			}
		}
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	if(dwTimerID == IDI_PERSONAL_BEFORE_BEGAIN_TIME  || dwTimerID ==  IDI_PERSONAL_AFTER_CREATE_ROOM_TIME)
	{
		...
		if (m_cbPayType == 0)
		{
			m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID, m_szPersonalTableID, m_cbPayType);//CANCELTABLE_REASON_ENFOCE
		}
		else
		{
			for (WORD i = 0; i < m_wChairCount; i++)
			{
				...
				m_pIMainServiceFrame->CancelCreateTable(pIServerUserItem->GetUserID(), m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_NOT_START, m_wTableID, m_szPersonalTableID, m_cbPayType);//CANCELTABLE_REASON_ENFOCE
			}
		}
		return true;
	}
	...
	case SUB_GF_GAME_OPTION:	//游戏配置
		{
			...
			if (m_PersonalRoomOption.cbPayType == 1 && pIServerUserItem != m_pTableOwnerUser)
			{
				m_pIMainServiceFrame->PayAATableFee(pIServerUserItem->GetUserID(), m_wTableID, m_szPersonalTableID,
					m_dwDrawCountLimit, m_dwDrawTimeLimit, pIServerUserItem->GetClientAddr());
			}
	...
	if (pIServerUserItem==pITableUserItem)
	{
		...
		{	
				...
				m_dwPersonalPlayCount != 0 && m_dwTableOwnerID == pIServerUserItem->GetUserID() && m_bPersonalLock == true )
			{
				...
				if (m_cbPayType == 0)
				{
					m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID, m_szPersonalTableID, m_cbPayType);
				}
				else
				{
					for (WORD i = 0; i < m_wChairCount; i++)
					{
						...
						m_pIMainServiceFrame->CancelCreateTable(pIServerUserItem->GetUserID(), m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_PLAYER, m_wTableID, m_szPersonalTableID, m_cbPayType);
					}
				}
			}
...
bool CTableFrame::CancelTableRequest(DWORD dwUserID, WORD dwChairID)
{
	...
	if(m_wUserCount == 1 && pUserItem->GetUserID() == m_dwTableOwnerID)//m_PersonalTableParameter.cbIsJoinGame)
	{
		...
		if (m_cbPayType == 0)
		{
			m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_ENFOCE, m_wTableID, m_szPersonalTableID, m_cbPayType);
		}
		else
		{
			for (WORD i = 0; i < m_wChairCount; i++)
			{
				...
				m_pIMainServiceFrame->CancelCreateTable(pIServerUserItem->GetUserID(), m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_ENFOCE, m_wTableID, m_szPersonalTableID, m_cbPayType);
			}
		}
...
VOID CTableFrame::SetPersonalTableParameter(tagPersonalTableParameter  PersonalTableParameter,  tagPersonalRoomOption PersonalRoomOption )
{
	...
	m_cbPayType = PersonalTableParameter.cbPayType;
...
void CTableFrame::HostDissumeGame(BOOL bIsEnforce )
{
	...
	if(!bIsEnforce)
	{
		...
		else
		{
			...
			if (m_cbPayType == 0)
			{
				m_pIMainServiceFrame->CancelCreateTable(m_dwTableOwnerID, m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_OVER_TIME, m_wTableID, m_szPersonalTableID, m_cbPayType);
			}
			else
			{
				for (WORD i = 0; i < m_wChairCount; i++)
				{
					...
					m_pIMainServiceFrame->CancelCreateTable(pIServerUserItem->GetUserID(), m_dwDrawCountLimit, m_dwDrawTimeLimit, CANCELTABLE_REASON_OVER_TIME, m_wTableID, m_szPersonalTableID, m_cbPayType);
				}
			}
		}
	}
...
VOID CTableFrame::SetGameRule(byte * cbRule,  int nLen)
{
	...
	m_pITableFrameSink->SetGameRule(cbRule, nLen);
}
