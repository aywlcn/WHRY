...
bool CAttemperEngineSink::OnDBCreateSucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	PersonalTableParameter.cbPayType = pCreateSuccess->cbPayType;
	...
	if (pCreateSuccess->cbIsJoinGame)
	{
		...
		if(pTableFrame->PerformSitDownAction(wChairID, pIServerUserItem, pCreateSuccess->szPassword)== FALSE)
		{
			...
			CancelCreateTable.cbPayType = pCreateSuccess->cbPayType;
	...
	CS_CreateTable.PersonalTable.cbPayType = pCreateSuccess->cbPayType;
	...
	case SUB_CS_S_CREATE_TABLE_RESULT:		//创建结果
		{
			...
			CreateSuccess.cbPayType = PersonalTableParameter.cbPayType;
			...
			CreateRecord.cbPayType = pCreateTableResult->PersonalTable.cbPayType;
...
//取消创建
VOID CAttemperEngineSink::CancelCreateTable(DWORD dwUserID, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwReason, WORD wTableID, TCHAR * szRoomID, BYTE cbPayType)
{
	//退还费用
	DBR_GR_CancelCreateTable CancelCreateTable;
	ZeroMemory(&CancelCreateTable, sizeof(DBR_GR_CancelCreateTable));
	...
	CancelCreateTable.cbPayType = cbPayType;
...
//AA付费
VOID CAttemperEngineSink::PayAATableFee(DWORD dwUserID, WORD wTableID, TCHAR * szRoomID, DWORD dwDrawCountLimit, DWORD dwDrawTimeLimit, DWORD dwClientAddr)
{
	//投递数据
	DBR_GR_PayAATableInfo payInfo;
	ZeroMemory(&payInfo, sizeof(DBR_GR_PayAATableInfo));

	payInfo.dwUserID = dwUserID;
	payInfo.dwTableID = wTableID;
	lstrcpyn(payInfo.szRoomID, szRoomID, sizeof(payInfo.szRoomID));
	payInfo.dwDrawCountLimit = dwDrawCountLimit;
	payInfo.dwDrawTimeLimit = dwDrawTimeLimit;
	payInfo.dwClientAddr = dwClientAddr;

	m_pIDBCorrespondManager->PostDataBaseRequest(0, DBR_GR_PAY_AA_TABLE_FEE, 0, &payInfo, sizeof(payInfo));
