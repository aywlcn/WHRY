	...
	try
	{
		...
		if(lResultCode == DB_SUCCESS)
		{
			...
			while (m_PlatformDBModule->IsRecordsetEnd()==false)
			{
				...
				pPersonalTableParameter->lAAFeeScore = m_PlatformDBAide.GetValue_LONGLONG(TEXT("AATableFee"));
	...
	try
	{
		...
		for (int i = 0; i < MAX_CREATE_PERSONAL_ROOM; i++)
		{
			...
			if (DB_SUCCESS == lResultCode)
			{
				...
				PersonalRoomInfoList.PersonalRoomInfo[i].cbPayType = m_TreasureDBAide.GetValue_BYTE(TEXT("PayOwnerOrAA"));
