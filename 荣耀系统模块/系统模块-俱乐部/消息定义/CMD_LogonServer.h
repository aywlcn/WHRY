...
//群
#define MDM_MB_GROUP_TABLE					500									//群
#define SUB_MB_CREATE_GROUP					1									//创建群
#define SUB_MB_HANDLE_ATTACH_GROUP			2									//处理加入群的请求
#define SUB_MB_GROUP_LIST					3									//获取创建的群列表
#define SUB_MB_GROUP_MEMBER					4								    //群成员
#define SUB_MB_DELETE_GROUP_MEMBER			5								    //删除群成员
#define SUB_MB_MODIFY_GROUP_NAME			6									//修改群名称
#define SUB_MB_DISSOLVED_GROUP				7									//解散群
#define SUB_MB_GROUP_ROOM_LIST				8									//群房间列表
#define SUB_MB_APPLY_ATTACH_GROUP			9									//申请加入群
#define SUB_MB_GET_APPLY_GROUP_LIST			10									//获取申请的群列表


#define SUB_MB_CREATE_GROUP_RESULT			21									//创建群的结果：成功or 失败


//创建群
struct CMD_GR_CreateGroup
{
	DWORD   dwUserID;
	DWORD   dwGameID;
	TCHAR	szGroupName[LEN_NICKNAME];
...
//创建成功
struct CMD_GR_CreateGroupSuccess
{
	DWORD   dwUserID;
...
//创建失败
struct CMD_GR_CreateGroupFailure
{
	INT							lErrorCode;							//错误代码
	TCHAR						szDescribeString[128];				//描述消息
};
...
struct tagGroupInfo_bk
{
	...
	bool operator < (const tagGroupInfo_bk& rhl)const
	{
		return dwGameID < rhl.dwGameID;
	}
...
//返回创建的群
struct CMD_CS_C_CreateGroup
{
	TCHAR szGroupID[GROUP_ID_LEN];
};

//返回的创建群列表
struct CMD_CS_C_GroupList
{
	//连接参数
	tagGroupInfo tagGroup[10];
};


//处理加入群请求返回
struct CMD_CS_C_HandleAttachGroup
{
	//连接参数
	BOOL bSuccessed;
...
//群成员返回
struct CMD_CS_C_GroupMember
{
	tagGroupMember tMemberList[200];
};


//删除群成员
struct CMD_CS_C_DeleteGroupMember
{
	BOOL bSuccessed;
	TCHAR szDescribeString[128];
};
...
//修改群名称
struct CMD_CS_C_ModifyGroupName
{
	BOOL bSuccessed;
	TCHAR szDescribeString[128];
};
...
//解散群
struct CMD_CS_C_DissolvedGroup
{
	BOOL bSuccessed;
	TCHAR szDescribeString[128];
};

struct tagGroupRoomList
{
	...
	BYTE  byGroupBase;
	BYTE  byMaxTimes;
	BYTE  byStartGameEntry;
	BYTE  byShangZhuangScore;
...
// 群房间列表
struct CMD_CS_C_GroupRoomList
{
	tagGroupRoomList tGroupRoomList[30];
};


// 申请加入群
struct CMD_CS_C_ApplyAttachGroup
{
	BOOL bSuccessed;
	TCHAR szDescribeString[128];
};
...
struct CMD_CS_C_GetApplyGroupList
{
	tagApplyGroupList tGroupList[100];
};
