	...
	case SUB_CS_C_CREATE_TABLE:		//创建桌子
		{
			...
			lstrcpyn(CreateTableResult.PersonalTable.szGroupID, pCreateTable->PersonalTable.szGroupID, CountArray(CreateTableResult.PersonalTable.szGroupID));
...
bool CAttemperEngineSink::OnTCPNetworkMainGroupService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_CS_C_CREATE_GROUP:		//创建群
		{
			//校验数据
			ASSERT(wDataSize == sizeof(CMD_GR_CreateGroupInfo));
			if (wDataSize != sizeof(CMD_GR_CreateGroupInfo)) return false;

			CMD_GR_CreateGroupInfo* pCreateGroup = (CMD_GR_CreateGroupInfo*)pData;

			//生成ID
			TCHAR szGroupID[5] = TEXT("");
			m_GlobalInfoManager.RandGroupID(szGroupID, 5);

			lstrcpyn(pCreateGroup->szGroupID, szGroupID, CountArray(pCreateGroup->szGroupID));

			//将群信息加入Map表
			tagGroupInfo tGroup;
			ZeroMemory(&tGroup,sizeof(tGroup));
			tGroup.dwMemberCount = 1;
			tGroup.dwRoomCount = 0;
			tGroup.dwGameID = pCreateGroup->dwGameID;
			lstrcpyn(tGroup.szGroupID, szGroupID, CountArray(tGroup.szGroupID));
			lstrcpyn(tGroup.szGroupName, pCreateGroup->szGroupName, CountArray(tGroup.szGroupName));
			m_GlobalInfoManager.AddServerGroup(szGroupID, tGroup);

			//构造数据
			CMD_GR_CreateGroupResult CreateTableResult;

			ZeroMemory(&CreateTableResult, sizeof(CMD_GR_CreateGroupResult));
			memcpy(&CreateTableResult, pCreateGroup,sizeof(CreateTableResult));
			CreateTableResult.dwSocketID = dwSocketID;
			m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_GROUP_SERVICE, SUB_CS_S_CREATE_GROUP_RESULT, &CreateTableResult, sizeof(CMD_GR_CreateGroupResult));
			return true;
		}
	case SUB_CS_C_HANDLE_ATTACH_GROUP:		//处理加入群请求
		{
			//校验数据
			ASSERT(wDataSize == sizeof(CMD_GR_HandleAttachGroup));
			if (wDataSize != sizeof(CMD_GR_HandleAttachGroup)) return false;

			CMD_GR_HandleAttachGroup* pCreateGroup = (CMD_GR_HandleAttachGroup*)pData;

			m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_GROUP_SERVICE, SUB_CS_S_HANDLE_ATTACH_GROUP_RESULT, &pCreateGroup, sizeof(CMD_GR_HandleAttachGroup));
			return true;
		}
	case SUB_CS_C_GROUP_LIST:		//获取创建的群列表
		{
			return true;
		}
	case SUB_CS_C_GROUP_MEMBER:		//获取群成员
		{
			return true;
		}
	case SUB_CS_C_DELETE_GROUP_MEMBER:		//删除群成员
		{
			return true;
		}
	case SUB_CS_C_MODIFY_GROUP_NAME:		//修改群名称
		{
			return true;
		}
	case SUB_CS_C_DISSOLVED_GROUP:		//解散群
		{
			return true;
		}
	case SUB_CS_C_GROUP_ROOM_LIST:		//获取群房间列表
		{
			return true;
	}
	case SUB_CS_C_APPLY_ATTACH_GROUP:		//申请入群
		{
			return true;
		}
	case SUB_CS_C_GET_APPLY_GROUP_LIST:		//获取申请入群列表
		{
			return true;
		}
	}
