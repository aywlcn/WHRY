...
CGlobalInfoManager::CGlobalInfoManager()
{
	...
	//群
	m_MapGroupInfo.InitHashTable(MAX_SERVER);
	return;
}
...
//生成群ID
VOID CGlobalInfoManager::RandGroupID(LPTSTR pszGroupID, WORD wMaxCount)
{
	//定义变量
	TCHAR szScource[11] = TEXT("0123456789");
	TCHAR* lpszTmp = new TCHAR[wMaxCount + 1];
	...
	while (bExit)
	{
		...
		if (m_MapGroupInfo.Lookup(lpszTmp, pGroupInfo) == TRUE)
			bExit = true;
		else
			bExit = false;
	}
	...
	//字符转换
	CW2CT strSrcData(lpszTmp);
	lstrcpyn(pszGroupID, strSrcData, wMaxCount + 1);
...
//添加群
bool CGlobalInfoManager::AddServerGroup(CString strGroupID, tagGroupInfo groupInfo)
{
	//添加桌子
	tagGroupInfo* pGroup = NULL;
	if (m_MapGroupInfo.Lookup(strGroupID, pGroup) == FALSE)
	{
		pGroup = new tagGroupInfo;

		m_MapGroupInfo[strGroupID] = pGroup;
		CopyMemory(pGroup, &groupInfo, sizeof(tagGroupInfo));
	}
	else
	{
		return false;
