...
bool CAttemperEngineSink::OnDBCreateSucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	lstrcpyn(CS_CreateTable.PersonalTable.szGroupID, pCreateSuccess->szGroupID, CountArray(CS_CreateTable.PersonalTable.szGroupID));
	...
	case SUB_CS_S_CREATE_TABLE_RESULT:		//创建结果
		{
			...
			lstrcpyn(CreateRecord.szGroupID, pCreateTableResult->PersonalTable.szGroupID, CountArray(CreateRecord.szGroupID));
...
bool CAttemperEngineSink::OnTCPNetworkSubCreateTable(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	for(INT_PTR i = 0; i < nSize; ++i)
	{
		...
		if(pTableFrame->GetNullChairCount() == pTableFrame->GetChairCount() && pTableFrame->IsPersonalTableLocked() == false)
		{
			...
			lstrcpyn(CreateTable.szGroupID, pCreateTable->szGroupID, CountArray(CreateTable.szGroupID));
