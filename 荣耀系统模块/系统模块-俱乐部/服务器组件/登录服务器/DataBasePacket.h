...
//////////////////////////////////////////////////////////////////////////////////
#define DBR_GP_CREATE_GROUP				1400							//创建群
#define DBR_GP_HANDLE_ATTACH_GROUP		1401							//处理入群的请求
#define DBR_GP_GET_GROUP_LIST			1402							//获取群列表
#define DBR_GP_GET_GROUP_MEMBER_LIST	1403							//获取群成员列表
#define DBR_GP_DELETE_GROUP_MEMBER		1404							//删除群成员
#define DBR_GP_MODIFY_GROUP_NAME		1405							//修改群名称
#define DBR_GP_DISSOLVED_GROUP			1406							//解散群
#define DBR_GP_GET_GROUP_ROOM_LIST		1407							//获取群房间列表
#define DBR_GP_APPLY_ATTACH_GROUP		1408							//申请入群
#define DBR_GP_GET_APPLY_GROUP_LIST		1409							//获取申请的群列表

//群
#define DBO_GP_CREATE_GROUP_SUCCESS				1450							//创建成功
#define DBO_GP_CREATE_GROUP_FAILURE				1451							//创建失败
#define DBO_GP_HANDLE_ATTACH_GROUP_RESULT		1452							//处理入群的请求
#define DBO_GP_GET_GROUP_LIST_RESULT			1453							//获取群列表
#define DBO_GP_GET_GROUP_MEMBER_LIST_RESULT		1454							//获取群成员列表
#define DBO_GP_DELETE_GROUP_MEMBER_RESULT		1455							//删除群成员
#define DBO_GP_MODIFY_GROUP_NAME_RESULT			1456							//修改群名称
#define DBO_GP_DISSOLVED_GROUP_RESULT			1457							//解散群
#define DBO_GP_GET_GROUP_ROOM_LIST_RESULT		1458							//获取群房间列表
#define DBO_GP_APPLY_ATTACH_GROUP_RESULT		1459							//申请入群
#define DBO_GP_GET_APPLY_GROUP_LIST_RESULT		1460							//获取申请的群列表


//--------------request---------------
//创建群
struct DBR_GP_CreateGroup
{
	DWORD							dwSocketID;							//网络标识
	DWORD							dwClientAddr;						//连接地址

	DWORD							dwCreateUserID;						//创建者用户标识
	DWORD							dwGameID;							//游戏ID
	DWORD							dwMemberMaxLimit;					//群最大人数限制
	DWORD							dwCreateTime;						//创建时间
	TCHAR							szGroupID[GROUP_ID_LEN];			//群ID
	TCHAR							szGroupName[LEN_NICKNAME];			//群名字
	TCHAR							szNotice[LEN_MESSAGE_CONTENT];		//群公告
};

//创建成功
struct DBO_GP_CreateGroupSuccess
{
	DWORD							dwUserID;							//用户标识
	TCHAR							szGroupID[GROUP_ID_LEN];			//群ID
	TCHAR							szGroupName[LEN_NICKNAME];			//群名字
...
//创建失败
struct DBO_GP_CreateGroupFailure
{
	LONG							lErrorCode;							//错误代码
	TCHAR							szDescribeString[128];				//描述消息
};
...
// 处理加入群请求
struct DBO_GP_HandleAttachGroup
{
	BYTE	byAgreeOrRefuse;	// 同意或拒绝
	DWORD   dwReqUserID;		// 请求玩家的UserID
	DWORD   dwReqGameID;		// 请求玩家的GameID
	DWORD	dwGroupID;			// 群id
};

//群列表
struct DBO_GP_GroupList
{
	DWORD dwUserID;
};

// 申请加入群
struct DBO_GP_ApplyAttachGroup
{
	DWORD   dwApplyUserID;		//申请的玩家userid
	DWORD   dwApplyGameID;		//申请的玩家gameid
	DWORD	dwGroupID;			//群id
};


// 获取申请的群列表
struct DBO_GP_GetApplyGroupList
{
	DWORD	dwGroupID;			//群id
};

//获取群成员
struct DBO_GP_GroupMember
{
	DWORD dwGroupID;
};

//删除群成员
struct DBO_GP_DeleteGroupMember
{
	DWORD dwGroupID;		//群id
	DWORD dwDestGameID;		//目标玩家gameid
	DWORD dwDestUserID;		//目标玩家userid
};

//修改群名称
struct DBO_GP_ModifyGroupName
{
	DWORD dwGroupID;					//群id
	TCHAR szModifyName[LEN_NICKNAME];   //修改的名称
};

//解散群
struct DBO_GP_DissolvedGroup
{
	DWORD dwGroupID;					//群id
};

//群房间列表
struct DBO_GP_GroupRoomList
{
	DWORD dwGroupID;					//群id
};
...
//处理入群申请
struct DBO_GP_HandleAttachGroupResult
{
	bool							bSuccessed;							//成功标识
	TCHAR							szDescribeString[128];				//描述消息
};
...
//获取群列表结果
struct DBO_GP_GroupListResult
{
	WORD							wGroupCount;
	tagGroupInfo					tGroupList[100];		//群信息
	TCHAR							szDescribeString[128];				//描述消息
};

//获取群成员列表结果
struct DBO_GP_GroupMemberListResult
{
	WORD							wMemberCount;
	tagGroupMember					tMemberList[200];		//群成员信息
	TCHAR							szDescribeString[128];				//描述消息
};

//删除群成员结果
struct DBO_GP_DeleteGroupMemberResult
{
	bool							bSuccessed;							//成功标识
	TCHAR							szDescribeString[128];				//描述消息
};
...
//修改群名称结果
struct DBO_GP_ModifyGroupNameResult
{
	bool							bSuccessed;							//成功标识
	TCHAR							szDescribeString[128];				//描述消息
};
...
//解散群结果
struct DBO_GP_DissolvedGroupResult
{
	bool							bSuccessed;							//成功标识
	TCHAR							szDescribeString[128];				//描述消息
};
...
//获取群房间列表结果
struct DBO_GP_GroupRoomListResult
{
	WORD							wRoomCount;
	tagGroupRoomList				tRoomList[25];		//群成员信息
	TCHAR							szDescribeString[128];				//描述消息
};

//申请入群结果
struct DBO_GP_ApplyAttachGroupResult
{
	bool							bSuccessed;							//成功标识
	TCHAR							szDescribeString[128];				//描述消息
};
...
//获取申请入群的列表结果
struct DBO_GP_GetApplyGroupResult
{
	tagApplyGroupList				tApplyGroupList[100];		//群成员信息
	TCHAR							szDescribeString[128];				//描述消息
};
