...
bool CAttemperEngineSink::OnEventTCPNetworkRead(TCP_Command Command, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case MDM_MB_GROUP_TABLE:
		{
			return OnTCPNetworkMainMBGroupTable(Command.wSubCmdID, pData, wDataSize, dwSocketID);
		}
	}
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GP_CREATE_GROUP_SUCCESS:
		{
			return OnDBCreateGroupSucess(dwContextID, pData, wDataSize);
		}
	case DBO_GP_CREATE_GROUP_FAILURE:
		{
			return OnDBCreateGroupFailure(dwContextID, pData, wDataSize);
		}
	case DBO_GP_HANDLE_ATTACH_GROUP_RESULT:
		{
			return OnDBHandleAttachGroupResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_GET_GROUP_LIST_RESULT:
		{
			return OnDBGetGroupListResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_GET_GROUP_MEMBER_LIST_RESULT:
		{
			return OnDBGetGroupMemberListResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_DELETE_GROUP_MEMBER_RESULT:
		{
			return OnDBDeleteGroupMemberResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_MODIFY_GROUP_NAME_RESULT:
		{
			return OnDBModifyGroupNameResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_DISSOLVED_GROUP_RESULT:
		{
			return OnDBDissolvedGroupResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_GET_GROUP_ROOM_LIST_RESULT:
		{
			return OnDBGetGroupRoomListResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_APPLY_ATTACH_GROUP_RESULT:
		{
			return OnDBApplyAttachGroupResult(dwContextID, pData, wDataSize);
		}
	case DBO_GP_GET_APPLY_GROUP_LIST_RESULT:
		{
			return OnDBGetApplyGroupListResult(dwContextID, pData, wDataSize);
		}
	}
...
bool CAttemperEngineSink::OnEventTCPSocketRead(WORD wServiceID, TCP_Command Command, VOID * pData, WORD wDataSize)
{
	...
	if (wServiceID==NETWORK_CORRESPOND)
	{
		...
		case MDM_CS_GROUP_SERVICE:
			{
				return OnTCPNetworkMainGroupService(Command.wSubCmdID, pData, wDataSize);
			}
		}
	}
...
//群服务
bool CAttemperEngineSink::OnTCPNetworkMainGroupService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	switch (wSubCmdID)
	{
	case SUB_CS_S_CREATE_GROUP_RESULT:		//创建群
		{
			//校验数据
			ASSERT(wDataSize == sizeof(CMD_GR_CreateGroupResult));
			if (wDataSize != sizeof(CMD_GR_CreateGroupResult)) return false;
			CMD_GR_CreateGroupResult* pCreateGroup = (CMD_GR_CreateGroupResult*)pData;

			DBR_GP_CreateGroup dbrData;
			ZeroMemory(&dbrData,sizeof(dbrData));
			memcpy(&dbrData,pCreateGroup,sizeof(dbrData));

			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_CREATE_GROUP, pCreateGroup->dwSocketID, &dbrData, sizeof(DBR_GP_CreateGroup));
			return true;
		}
	case SUB_CS_S_HANDLE_ATTACH_GROUP_RESULT:		//处理加入群请求
		{
			return true;
		}
	case SUB_CS_S_GROUP_LIST_RESULT:		//获取创建的群列表
		{
			return true;
		}
	case SUB_CS_S_GROUP_MEMBER_RESULT:		//获取群成员
		{
			return true;
		}
	case SUB_CS_S_DELETE_GROUP_MEMBER_RESULT:		//删除群成员
		{
			return true;
		}
	case SUB_CS_S_MODIFY_GROUP_NAME_RESULT:		//修改群名称
		{
			return true;
		}
	case SUB_CS_S_DISSOLVED_GROUP_RESULT:		//解散群
		{
			return true;
		}
	case SUB_CS_S_GROUP_ROOM_LIST_RESULT:		//获取群房间列表
		{
			return true;
		}
	case SUB_CS_S_APPLY_ATTACH_GROUP_RESULT:		//申请入群
		{
			return true;
		}
	case SUB_CS_S_GET_APPLY_GROUP_LIST_RESULT:		//获取申请入群列表
		{
			return true;
		}
	}
...
//群相关消息:客户端向服务器所发
bool CAttemperEngineSink::OnTCPNetworkMainMBGroupTable(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_MB_CREATE_GROUP:
		{
			return OnTCPNetworkSubCreateGroup(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_HANDLE_ATTACH_GROUP:
		{
			return OnTCPNetworkSubHandleAttachGroup(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_GROUP_LIST:
		{
			return OnTCPNetworkSubGetGroupList(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_GROUP_MEMBER:
		{
			return OnTCPNetworkSubGetGroupMemberList(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_DELETE_GROUP_MEMBER:
		{
			return OnTCPNetworkSubDelGroupMember(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_MODIFY_GROUP_NAME:
		{
			return OnTCPNetworkSubModifyGroupName(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_DISSOLVED_GROUP:
		{
			return OnTCPNetworkSubDissolvedGroup(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_GROUP_ROOM_LIST:
		{
			return OnTCPNetworkSubGetGroupRoomList(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_APPLY_ATTACH_GROUP:
		{
			return OnTCPNetworkSubApplyAttachGroup(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_GET_APPLY_GROUP_LIST:
		{
			return OnTCPNetworkSubGetApplyGroupList(pData, wDataSize, dwSocketID);
		}
	}
	return false;
}
...
//创建群
bool CAttemperEngineSink::OnTCPNetworkSubCreateGroup(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_CreateGroup));
	if (wDataSize != sizeof(CMD_GR_CreateGroup)) return false;

	//提取数据
	CMD_GR_CreateGroup * pCreateGroup = (CMD_GR_CreateGroup*)pData;
	ASSERT(pCreateGroup != NULL);

	//发送数据
	CMD_GR_CreateGroupInfo createGroup;
	ZeroMemory(&createGroup, sizeof(CMD_GR_CreateGroupInfo));
	createGroup.dwSocketID = dwSocketID;

	createGroup.dwCreateUserID = pCreateGroup->dwUserID;
	createGroup.dwGameID = pCreateGroup->dwGameID;
	createGroup.dwMemberMaxLimit = 100;
	createGroup.dwCreateTime = (DWORD)time(NULL);
	lstrcpyn(createGroup.szGroupName, pCreateGroup->szGroupName, sizeof(createGroup.szGroupName));

	//发送消息
	m_pITCPSocketService->SendData(MDM_CS_GROUP_SERVICE, SUB_CS_C_CREATE_GROUP, &createGroup, sizeof(CMD_GR_CreateGroupInfo));

	return true;
}
//处理加入群的请求 
bool CAttemperEngineSink::OnTCPNetworkSubHandleAttachGroup(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_HandleAttachGroup));
	if (wDataSize != sizeof(CMD_GR_HandleAttachGroup)) return false;

	//提取数据
	CMD_GR_HandleAttachGroup * pCmdData = (CMD_GR_HandleAttachGroup*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_HandleAttachGroup dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_HandleAttachGroup));
	dbrData.byAgreeOrRefuse = pCmdData->byAgreeOrRefuse;
	dbrData.dwReqUserID = pCmdData->dwReqUserID;
	dbrData.dwReqGameID = pCmdData->dwReqGameID;
	dbrData.dwGroupID = pCmdData->dwGroupID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_HANDLE_ATTACH_GROUP, dwSocketID, &dbrData, sizeof(DBO_GP_HandleAttachGroup));
	return true;
}

//获取群列表
bool CAttemperEngineSink::OnTCPNetworkSubGetGroupList(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_GroupList));
	if (wDataSize != sizeof(CMD_GR_GroupList)) return false;

	//提取数据
	CMD_GR_GroupList * pCmdData = (CMD_GR_GroupList*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_GroupList dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_GroupList));
	dbrData.dwUserID = pCmdData->dwUserID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GET_GROUP_LIST, dwSocketID, &dbrData, sizeof(DBO_GP_GroupList));
	return true;
}
//获取群成员列表
bool CAttemperEngineSink::OnTCPNetworkSubGetGroupMemberList(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_GroupMember));
	if (wDataSize != sizeof(CMD_GR_GroupMember)) return false;

	//提取数据
	CMD_GR_GroupMember * pCmdData = (CMD_GR_GroupMember*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_GroupMember dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_GroupMember));
	dbrData.dwGroupID = pCmdData->dwGroupID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GET_GROUP_LIST, dwSocketID, &dbrData, sizeof(DBO_GP_GroupMember));
	return true;
}
//删除群成员
bool CAttemperEngineSink::OnTCPNetworkSubDelGroupMember(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_DeleteGroupMember));
	if (wDataSize != sizeof(CMD_GR_DeleteGroupMember)) return false;

	//提取数据
	CMD_GR_DeleteGroupMember * pCmdData = (CMD_GR_DeleteGroupMember*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_DeleteGroupMember dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_DeleteGroupMember));
	dbrData.dwGroupID = pCmdData->dwGroupID;
	dbrData.dwDestGameID = pCmdData->dwDestGameID;
	dbrData.dwDestUserID = pCmdData->dwDestUserID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_DELETE_GROUP_MEMBER, dwSocketID, &dbrData, sizeof(DBO_GP_DeleteGroupMember));
	return true;
}
//修改群名称
bool CAttemperEngineSink::OnTCPNetworkSubModifyGroupName(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_ModifyGroupName));
	if (wDataSize != sizeof(CMD_GR_ModifyGroupName)) return false;

	//提取数据
	CMD_GR_ModifyGroupName * pCmdData = (CMD_GR_ModifyGroupName*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_ModifyGroupName dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_ModifyGroupName));
	dbrData.dwGroupID = pCmdData->dwGroupID;
	lstrcpyn(dbrData.szModifyName, pCmdData->szModifyName, sizeof(dbrData.szModifyName));

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_MODIFY_GROUP_NAME, dwSocketID, &dbrData, sizeof(DBO_GP_ModifyGroupName));
	return true;
}
//解散群
bool CAttemperEngineSink::OnTCPNetworkSubDissolvedGroup(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_DissolvedGroup));
	if (wDataSize != sizeof(CMD_GR_DissolvedGroup)) return false;

	//提取数据
	CMD_GR_DissolvedGroup * pCmdData = (CMD_GR_DissolvedGroup*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_DissolvedGroup dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_DissolvedGroup));
	dbrData.dwGroupID = pCmdData->dwGroupID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_DISSOLVED_GROUP, dwSocketID, &dbrData, sizeof(DBO_GP_ModifyGroupName));
	return true;
}
//获取群房间列表
bool CAttemperEngineSink::OnTCPNetworkSubGetGroupRoomList(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_GroupRoomList));
	if (wDataSize != sizeof(CMD_GR_GroupRoomList)) return false;

	//提取数据
	CMD_GR_GroupRoomList * pCmdData = (CMD_GR_GroupRoomList*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_GroupRoomList dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_GroupRoomList));
	dbrData.dwGroupID = pCmdData->dwGroupID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GET_GROUP_ROOM_LIST, dwSocketID, &dbrData, sizeof(DBO_GP_GroupRoomList));
	return true;
}
//申请加入群
bool CAttemperEngineSink::OnTCPNetworkSubApplyAttachGroup(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_ApplyAttachGroup));
	if (wDataSize != sizeof(CMD_GR_ApplyAttachGroup)) return false;

	//提取数据
	CMD_GR_ApplyAttachGroup * pCmdData = (CMD_GR_ApplyAttachGroup*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_ApplyAttachGroup dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_ApplyAttachGroup));
	dbrData.dwApplyUserID = pCmdData->dwApplyUserID;
	dbrData.dwGroupID = pCmdData->dwGroupID;
	dbrData.dwApplyGameID = pCmdData->dwApplyGameID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_APPLY_ATTACH_GROUP, dwSocketID, &dbrData, sizeof(DBO_GP_ApplyAttachGroup));
	return true;
}
//获取申请的群列表
bool CAttemperEngineSink::OnTCPNetworkSubGetApplyGroupList(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//校验数据
	ASSERT(wDataSize == sizeof(CMD_GR_GetApplyGroupList));
	if (wDataSize != sizeof(CMD_GR_GetApplyGroupList)) return false;

	//提取数据
	CMD_GR_GetApplyGroupList * pCmdData = (CMD_GR_GetApplyGroupList*)pData;
	ASSERT(pCmdData != NULL);

	//投递数据
	DBO_GP_GetApplyGroupList dbrData;
	ZeroMemory(&dbrData, sizeof(DBO_GP_GetApplyGroupList));
	dbrData.dwGroupID = pCmdData->dwGroupID;

	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_GET_APPLY_GROUP_LIST, dwSocketID, &dbrData, sizeof(DBO_GP_GetApplyGroupList));
	return true;
}
...
bool CAttemperEngineSink::OnDBMBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	pCMDLogonSuccess->dwExperience=pDBOLogonSuccess->dwExperience;
	pCMDLogonSuccess->lLoveLiness=pDBOLogonSuccess->lLoveLiness;
	pCMDLogonSuccess->dwUserRight = pDBOLogonSuccess->dwUserRight;
	lstrcpyn(pCMDLogonSuccess->szAccounts,pDBOLogonSuccess->szAccounts,CountArray(pCMDLogonSuccess->szAccounts));
	lstrcpyn(pCMDLogonSuccess->szNickName,pDBOLogonSuccess->szNickName,CountArray(pCMDLogonSuccess->szNickName));
	lstrcpyn(pCMDLogonSuccess->szDynamicPass,pDBOLogonSuccess->szDynamicPass,CountArray(pCMDLogonSuccess->szDynamicPass));
...
//创建成功
bool CAttemperEngineSink::OnDBCreateGroupSucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_CreateGroupSuccess));
	if (wDataSize != sizeof(DBO_GP_CreateGroupSuccess)) return false;

	DBO_GP_CreateGroupSuccess* pCreateSuccess = (DBO_GP_CreateGroupSuccess*)pData;

	//构造数据
	CMD_CS_C_CreateGroup CreateResult;
	ZeroMemory(&CreateResult, sizeof(CMD_CS_C_CreateGroup));

	lstrcpyn(CreateResult.szGroupID, pCreateSuccess->szGroupID, sizeof(CreateResult.szGroupID));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_CREATE_GROUP_RESULT, &CreateResult, sizeof(CMD_GR_CreateGroupSuccess));
	return true;
}

//创建失败
bool CAttemperEngineSink::OnDBCreateGroupFailure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_CreateGroupFailure));
	if (wDataSize != sizeof(DBO_GP_CreateGroupFailure)) return false;

	DBO_GP_CreateGroupFailure* pCreateFailure = (DBO_GP_CreateGroupFailure*)pData;

	//构造数据
	CMD_CS_C_CreateGroup CreateFailure;
	ZeroMemory(&CreateFailure, sizeof(CMD_CS_C_CreateGroup));

	lstrcpyn(CreateFailure.szGroupID, 0, CountArray(CreateFailure.szGroupID));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_CREATE_GROUP_RESULT, &CreateFailure, sizeof(CMD_CS_C_CreateGroup));
	return true;
}

//申请入群处理结果
bool CAttemperEngineSink::OnDBHandleAttachGroupResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_HandleAttachGroupResult));
	if (wDataSize != sizeof(DBO_GP_HandleAttachGroupResult)) return false;

	DBO_GP_HandleAttachGroupResult* pRst = (DBO_GP_HandleAttachGroupResult*)pData;

	//构造数据
	CMD_CS_C_HandleAttachGroup sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_HandleAttachGroup));
	sendData.bSuccessed = pRst->bSuccessed;
	lstrcpyn(sendData.szDescribeString, pRst->szDescribeString, CountArray(sendData.szDescribeString));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_CREATE_GROUP_RESULT, &sendData, sizeof(CMD_CS_C_HandleAttachGroup));
	return true;
}
//获取群列表
bool CAttemperEngineSink::OnDBGetGroupListResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_GetApplyGroupResult));
	if (wDataSize != sizeof(DBO_GP_GetApplyGroupResult)) return false;

	DBO_GP_GetApplyGroupResult* pRst = (DBO_GP_GetApplyGroupResult*)pData;

	//构造数据
	CMD_CS_C_GetApplyGroupList sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_GetApplyGroupList));
	memcpy(sendData.tGroupList, pRst->tApplyGroupList, sizeof(sendData.tGroupList));

	//发送应答给客户端
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_GROUP_LIST, &sendData, sizeof(CMD_CS_C_GetApplyGroupList));
	return true;
}

//获取群成员列表
bool CAttemperEngineSink::OnDBGetGroupMemberListResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_GroupMemberListResult));
	if (wDataSize != sizeof(DBO_GP_GroupMemberListResult)) return false;

	DBO_GP_GroupMemberListResult* pRst = (DBO_GP_GroupMemberListResult*)pData;

	//构造数据
	CMD_CS_C_GroupMember sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_GroupMember));
	memcpy(sendData.tMemberList, pRst->tMemberList, sizeof(sendData.tMemberList));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_GROUP_MEMBER, &sendData, sizeof(CMD_CS_C_GroupMember));
	return true;
}
//删除群成员
bool CAttemperEngineSink::OnDBDeleteGroupMemberResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_DeleteGroupMemberResult));
	if (wDataSize != sizeof(DBO_GP_DeleteGroupMemberResult)) return false;

	DBO_GP_DeleteGroupMemberResult* pRst = (DBO_GP_DeleteGroupMemberResult*)pData;

	//构造数据
	CMD_CS_C_DeleteGroupMember sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_DeleteGroupMember));
	sendData.bSuccessed = pRst->bSuccessed;
	lstrcpyn(sendData.szDescribeString, pRst->szDescribeString, CountArray(sendData.szDescribeString));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_DELETE_GROUP_MEMBER, &sendData, sizeof(CMD_CS_C_DeleteGroupMember));
	return true;
}
//修改群名称
bool CAttemperEngineSink::OnDBModifyGroupNameResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_ModifyGroupNameResult));
	if (wDataSize != sizeof(DBO_GP_ModifyGroupNameResult)) return false;

	DBO_GP_ModifyGroupNameResult* pRst = (DBO_GP_ModifyGroupNameResult*)pData;
...
//解散群
bool CAttemperEngineSink::OnDBDissolvedGroupResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_DissolvedGroupResult));
	if (wDataSize != sizeof(DBO_GP_DissolvedGroupResult)) return false;

	DBO_GP_DissolvedGroupResult* pRst = (DBO_GP_DissolvedGroupResult*)pData;

	//构造数据
	CMD_CS_C_DissolvedGroup sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_DissolvedGroup));
	sendData.bSuccessed = pRst->bSuccessed;
	lstrcpyn(sendData.szDescribeString, pRst->szDescribeString, CountArray(sendData.szDescribeString));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_CREATE_GROUP_RESULT, &sendData, sizeof(CMD_CS_C_DissolvedGroup));
	return true;
}

//获取群房间列表
bool CAttemperEngineSink::OnDBGetGroupRoomListResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_GroupRoomListResult));
	if (wDataSize != sizeof(DBO_GP_GroupRoomListResult)) return false;

	DBO_GP_GroupRoomListResult* pRst = (DBO_GP_GroupRoomListResult*)pData;

	//构造数据
	CMD_CS_C_GroupRoomList sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_GroupRoomList));
	memcpy(sendData.tGroupRoomList,pRst->tRoomList,sizeof(sendData.tGroupRoomList));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_GROUP_ROOM_LIST, &sendData, sizeof(CMD_CS_C_GroupRoomList));
	return true;
}

//申请入群
bool CAttemperEngineSink::OnDBApplyAttachGroupResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_ApplyAttachGroupResult));
	if (wDataSize != sizeof(DBO_GP_ApplyAttachGroupResult)) return false;

	DBO_GP_ApplyAttachGroupResult* pRst = (DBO_GP_ApplyAttachGroupResult*)pData;

	//构造数据
	CMD_CS_C_ApplyAttachGroup sendData;
	ZeroMemory(&sendData, sizeof(CMD_CS_C_ApplyAttachGroup));
	sendData.bSuccessed = pRst->bSuccessed;
	lstrcpyn(sendData.szDescribeString, pRst->szDescribeString, CountArray(sendData.szDescribeString));

	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_APPLY_ATTACH_GROUP, &sendData, sizeof(CMD_CS_C_ApplyAttachGroup));
	return true;
}

//获取申请的群列表
bool CAttemperEngineSink::OnDBGetApplyGroupListResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//数据校验
	ASSERT(wDataSize == sizeof(DBO_GP_GetApplyGroupResult));
	if (wDataSize != sizeof(DBO_GP_GetApplyGroupResult)) return false;
	...
	//发送数据
	m_pITCPNetworkEngine->SendData(dwContextID, MDM_MB_GROUP_TABLE, SUB_MB_GET_APPLY_GROUP_LIST, &sendData, sizeof(CMD_CS_C_GetApplyGroupList));
	return true;
}
