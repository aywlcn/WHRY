...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_CREATE_GROUP:
		{
			return OnRequestCreateGroup(dwContextID, pData, wDataSize);
		}
	case DBR_GP_HANDLE_ATTACH_GROUP:
		{
			return OnRequestHandleAttachGroup(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GET_GROUP_LIST:
		{
			return OnRequestQueryGroupTable(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GET_GROUP_MEMBER_LIST:
		{
			return OnRequestQueryGroupMember(dwContextID, pData, wDataSize);
		}
	case DBR_GP_DELETE_GROUP_MEMBER:
		{
			return OnRequestDeleteGroupMember(dwContextID, pData, wDataSize);
		}
	case DBR_GP_MODIFY_GROUP_NAME:
		{
			return OnRequestModifyGroupName(dwContextID, pData, wDataSize);
		}
	case DBR_GP_DISSOLVED_GROUP:
		{
			return OnRequestDissolvedGroup(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GET_GROUP_ROOM_LIST:
		{
			return OnRequestQueryGroupRoom(dwContextID, pData, wDataSize);
		}
	case DBR_GP_APPLY_ATTACH_GROUP:
		{
			return OnRequestApplyAttachGroup(dwContextID, pData, wDataSize);
		}
	case DBR_GP_GET_APPLY_GROUP_LIST:
		{
			return OnRequestQueryApplyGroup(dwContextID, pData, wDataSize);
		}
	}
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString,DWORD dwCheckUserRight, bool bMobileClient)
{
	...
	else
	{
		if (dwErrorCode==DB_SUCCESS)
		{
			...
			LogonSuccess.dwExperience=m_AccountsDBAide.GetValue_DWORD(TEXT("Experience"));
			LogonSuccess.lLoveLiness=m_AccountsDBAide.GetValue_LONGLONG(TEXT("LoveLiness"));
			LogonSuccess.dwUserRight = m_AccountsDBAide.GetValue_DWORD(TEXT("UserRight"));
			m_AccountsDBAide.GetValue_String(TEXT("Accounts"),LogonSuccess.szAccounts,CountArray(LogonSuccess.szAccounts));
			m_AccountsDBAide.GetValue_String(TEXT("NickName"),LogonSuccess.szNickName,CountArray(LogonSuccess.szNickName));
			m_AccountsDBAide.GetValue_String(TEXT("DynamicPass"),LogonSuccess.szDynamicPass,CountArray(LogonSuccess.szDynamicPass));
			m_AccountsDBAide.GetValue_String(TEXT("UnderWrite"),LogonSuccess.szUnderWrite,CountArray(LogonSuccess.szUnderWrite));
...
bool CDataBaseEngineSink::OnRequestCreateGroup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestCreateGroup"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBR_GP_CreateGroup));
		if (wDataSize != sizeof(DBR_GP_CreateGroup)) return false;

		DBR_GP_CreateGroup* pCreateGroup = (DBR_GP_CreateGroup*)pData;

		//转化地址
		TCHAR szClientAddr[16] = TEXT("");
		BYTE * pClientAddr = (BYTE *)&pCreateGroup->dwClientAddr;
		_sntprintf(szClientAddr, CountArray(szClientAddr), TEXT("%d.%d.%d.%d"), pClientAddr[0], pClientAddr[1], pClientAddr[2], pClientAddr[3]);

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwUserID"), pCreateGroup->dwCreateUserID);
		m_TreasureDBAide.AddParameter(TEXT("@szGroupID"), pCreateGroup->szGroupID);		
		m_TreasureDBAide.AddParameter(TEXT("@szGroupName"), pCreateGroup->szGroupName);
		m_TreasureDBAide.AddParameter(TEXT("@dwCreateTime"), pCreateGroup->dwCreateTime);
		m_TreasureDBAide.AddParameter(TEXT("@szNotice"), pCreateGroup->szNotice);
		m_TreasureDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		...
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_InsertGroup"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_CreateGroupSuccess CreateSuccess;
			ZeroMemory(&CreateSuccess, sizeof(DBO_GP_CreateGroupSuccess));
			OutputDebugString(TEXT("ryp0tdt *** OnRequestCreateGroup 数据库创建成功"));
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_CREATE_GROUP_SUCCESS, dwContextID, &CreateSuccess, sizeof(DBO_GP_CreateGroupSuccess));
		}
		else
		{
			//获取参数
			...
			//构造结构
			DBO_GP_CreateGroupFailure CreateFailure;
			ZeroMemory(&CreateFailure, sizeof(DBO_GP_CreateGroupFailure));
			...
			//发送参数
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_CREATE_GROUP_FAILURE, dwContextID, &CreateFailure, sizeof(DBO_GP_CreateGroupFailure));
		}
	}
	catch (IDataBaseException * pIException)
	{
...
//处理入群请求
bool CDataBaseEngineSink::OnRequestHandleAttachGroup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestHandleAttachGroup"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_HandleAttachGroup));
		if (wDataSize != sizeof(DBO_GP_HandleAttachGroup)) return false;

		DBO_GP_HandleAttachGroup* pDbData = (DBO_GP_HandleAttachGroup*)pData;
		//构造参数
		m_PlatformDBAide.ResetParameter();
		m_PlatformDBAide.AddParameter(TEXT("@dwUserID"), pDbData->dwReqUserID);
		m_PlatformDBAide.AddParameter(TEXT("@szGroupID"), pDbData->dwGroupID);
		...
		//拒绝，删除申请表中的申请记录
		if (0 == pDbData->byAgreeOrRefuse)
		{
			if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_DeleteGroupApply"), true) == DB_SUCCESS)
			{
				//变量定义
				DBO_GP_HandleAttachGroupResult sendData;
				ZeroMemory(&sendData, sizeof(DBO_GP_HandleAttachGroupResult));
				sendData.bSuccessed = true;

				OutputDebugString(TEXT("ryp0tdt *** GSP_GP_DeleteGroupApply 数据库删除成功"));
				//发送结果
				m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_HANDLE_ATTACH_GROUP_RESULT, dwContextID, &sendData, sizeof(DBO_GP_HandleAttachGroupResult));
			}
			else
			{
				//获取参数
				...
				DBO_GP_HandleAttachGroupResult sendData;
				ZeroMemory(&sendData, sizeof(DBO_GP_HandleAttachGroupResult));
				sendData.bSuccessed = false;

				//设置变量
				lstrcpyn(sendData.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(sendData.szDescribeString));

				//发送参数
				m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_HANDLE_ATTACH_GROUP_RESULT, dwContextID, &sendData, sizeof(DBO_GP_HandleAttachGroupResult));
			}
		}
		else//同意，则先删除申请表中的申请记录，再增加一条记录到群成员表中
		{
			//结果处理
			if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_DeleteGroupApply"), true) == DB_SUCCESS)
			{
				OutputDebugString(TEXT("ryp0tdt *** GSP_GP_DeleteGroupApply 数据库删除成功"));
				//结果处理
				if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_InsertMemToGroup"), true) == DB_SUCCESS)
				{
					//变量定义
					DBO_GP_HandleAttachGroupResult sendData;
					ZeroMemory(&sendData, sizeof(DBO_GP_HandleAttachGroupResult));
					...
					OutputDebugString(TEXT("ryp0tdt *** GSP_GP_InsertGroupMember 数据库插入成功"));
					//发送结果
					m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_HANDLE_ATTACH_GROUP_RESULT, dwContextID, &sendData, sizeof(DBO_GP_HandleAttachGroupResult));
				}
				else
...
//获取群列表
bool CDataBaseEngineSink::OnRequestQueryGroupTable(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestQueryGroupTable"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_GroupList));
		if (wDataSize != sizeof(DBO_GP_GroupList)) return false;

		DBO_GP_GroupList* pWriteData = (DBO_GP_GroupList*)pData;

		//构造参数
		m_PlatformDBAide.ResetParameter();
		m_PlatformDBAide.AddParameter(TEXT("@dwUserID"), pWriteData->dwUserID);
		...
		//执行成功
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_QueryGroup"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_GroupListResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_GroupListResult));
			tagGroupInfo* pGroupInfo = NULL;

			//变量定义
			while (m_PlatformDBModule->IsRecordsetEnd() == false)
			{
				//设置变量
				pGroupInfo = &sendData.tGroupList[sendData.wGroupCount++];

				TCHAR szGroupID[GROUP_ID_LEN] = { 0 };
				ZeroMemory(szGroupID, sizeof(szGroupID));
				m_PlatformDBAide.GetValue_String(TEXT("GroupID"), szGroupID, sizeof(szGroupID));

				if (szGroupID == NULL || szGroupID[0] == 0)
				{
					//移动记录
					m_PlatformDBModule->MoveToNext();
					continue;
				...
				TCHAR szGroupName[32] = { 0 };
				ZeroMemory(szGroupName, sizeof(szGroupName));
				m_PlatformDBAide.GetValue_String(TEXT("GroupName"), szGroupName, sizeof(szGroupName));

				if (szGroupName == NULL || szGroupName[0] == 0)
				{
					//移动记录
					m_PlatformDBModule->MoveToNext();
					continue;
				...
				//读取数据
				lstrcpyn(pGroupInfo->szGroupID,szGroupID,sizeof(pGroupInfo->szGroupID));
				lstrcpyn(pGroupInfo->szGroupName, szGroupName, sizeof(pGroupInfo->szGroupName));
			...
			//发送结果
			WORD wSendDataSize = sizeof(sendData)-sizeof(sendData.tGroupList);
			wSendDataSize += sizeof(sendData.tGroupList[0])*sendData.wGroupCount;
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_LIST_RESULT, dwContextID, &sendData, wSendDataSize);
		}
		else
		{
			//变量定义
			...
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_LIST_RESULT, dwContextID, &sendData, sizeof(sendData));
		}
	}
	catch (IDataBaseException * pIException)
	{
		...
		ZeroMemory(&sendData, sizeof(sendData));
		sendData.wGroupCount = 0;
		lstrcpyn(sendData.szDescribeString, TEXT("数据库异常，请稍后再试！"), CountArray(sendData.szDescribeString));
...
//获取群成员列表
bool CDataBaseEngineSink::OnRequestQueryGroupMember(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestQueryGroupMember"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_GroupMember));
		if (wDataSize != sizeof(DBO_GP_GroupMember)) return false;

		DBO_GP_GroupMember* pWriteData = (DBO_GP_GroupMember*)pData;

		//构造参数
		m_PlatformDBAide.ResetParameter();
		m_PlatformDBAide.AddParameter(TEXT("@dwGroupID"), pWriteData->dwGroupID);
		...
		//执行成功
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_QueryGroupMember"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_GroupMemberListResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_GroupMemberListResult));
			tagGroupMember* pMemberInfo = NULL;

			//变量定义
			while (m_PlatformDBModule->IsRecordsetEnd() == false)
			{
				//设置变量
				pMemberInfo = &sendData.tMemberList[sendData.wMemberCount++];

				pMemberInfo->dwUserID = m_PlatformDBAide.GetValue_DWORD(TEXT("UserID"));
				TCHAR szUserName[32] = { 0 };
				ZeroMemory(szUserName, sizeof(szUserName));
				m_PlatformDBAide.GetValue_String(TEXT("UserID"),szUserName,sizeof(szUserName));
				...
				lstrcpyn(pMemberInfo->szNickName,szUserName,sizeof(pMemberInfo->szNickName));
			...
			//发送结果
			WORD wSendDataSize = sizeof(sendData)-sizeof(sendData.tMemberList);
			wSendDataSize += sizeof(sendData.tMemberList[0])*sendData.wMemberCount;
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_MEMBER_LIST_RESULT, dwContextID, &sendData, wSendDataSize);
		}
		else
		{
			//变量定义
			...
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_MEMBER_LIST_RESULT, dwContextID, &sendData, sizeof(sendData));
		}
	}
	catch (IDataBaseException * pIException)
	{
...
//删除群成员:如果群成员正在游戏中，则待游戏结束后再删除，删除前，先查询
bool CDataBaseEngineSink::OnRequestDeleteGroupMember(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestDeleteGroupMember"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_DeleteGroupMember));
		if (wDataSize != sizeof(DBO_GP_DeleteGroupMember)) return false;

		DBO_GP_DeleteGroupMember* pWriteData = (DBO_GP_DeleteGroupMember*)pData;

		//构造参数
		m_PlatformDBAide.ResetParameter();
		m_PlatformDBAide.AddParameter(TEXT("@dwUserID"), pWriteData->dwDestUserID);
		m_PlatformDBAide.AddParameter(TEXT("@szGroupID"), pWriteData->dwGroupID);
		...
		//结果处理
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_DeleteGroupMember"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_DeleteGroupMemberResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_DeleteGroupMemberResult));
			sendData.bSuccessed = true;
			OutputDebugString(TEXT("ryp0tdt *** OnRequestDeleteGroupMember 删除成功"));
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_DELETE_GROUP_MEMBER_RESULT, dwContextID, &sendData, sizeof(DBO_GP_DeleteGroupMemberResult));
		}
		else
		{
			//获取参数
...
bool CDataBaseEngineSink::OnRequestModifyGroupName(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestModifyGroupName"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_ModifyGroupName));
		if (wDataSize != sizeof(DBO_GP_ModifyGroupName)) return false;

		DBO_GP_ModifyGroupName* pWriteData = (DBO_GP_ModifyGroupName*)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwGroupID"), pWriteData->dwGroupID);
		m_TreasureDBAide.AddParameter(TEXT("@szModifyName"), pWriteData->szModifyName);
		...
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_UpdateGroupName"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_ModifyGroupNameResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_ModifyGroupNameResult));
			sendData.bSuccessed = true;
			OutputDebugString(TEXT("ryp0tdt *** OnRequestDeleteGroupMember 修改成功"));
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_MODIFY_GROUP_NAME_RESULT, dwContextID, &sendData, sizeof(DBO_GP_ModifyGroupNameResult));
		}
		else
		{
			//获取参数
...
//解散群：解散群时，需要考虑所开的房间和正在游戏的房间
bool CDataBaseEngineSink::OnRequestDissolvedGroup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestDissolvedGroup"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_DissolvedGroup));
		if (wDataSize != sizeof(DBO_GP_DissolvedGroup)) return false;

		DBO_GP_DissolvedGroup* pWriteData = (DBO_GP_DissolvedGroup*)pData;
		...
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_DeleteGroupMember"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_DissolvedGroupResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_DissolvedGroupResult));
			sendData.bSuccessed = true;
			OutputDebugString(TEXT("ryp0tdt *** OnRequestDissolvedGroup 删除成功"));
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_DISSOLVED_GROUP_RESULT, dwContextID, &sendData, sizeof(DBO_GP_DissolvedGroupResult));
		}
		else
		{
			//获取参数
...
//获取群房间列表
bool CDataBaseEngineSink::OnRequestQueryGroupRoom(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestQueryGroupRoom"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_GroupRoomList));
		if (wDataSize != sizeof(DBO_GP_GroupRoomList)) return false;

		DBO_GP_GroupRoomList* pWriteData = (DBO_GP_GroupRoomList*)pData;
		...
		//执行成功
		if (m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GP_GroupRoomList"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_GroupRoomListResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_GroupRoomListResult));
			tagGroupRoomList* pRoomInfo = NULL;

			//变量定义
			while (m_PlatformDBModule->IsRecordsetEnd() == false)
			{
				//设置变量
				pRoomInfo = &sendData.tRoomList[sendData.wRoomCount++];
			...
			//发送结果
			WORD wSendDataSize = sizeof(sendData)-sizeof(sendData.tRoomList);
			wSendDataSize += sizeof(sendData.tRoomList[0])*sendData.wRoomCount;
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_ROOM_LIST_RESULT, dwContextID, &sendData, wSendDataSize);
		}
		else
		{
			//变量定义
			...
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_ROOM_LIST_RESULT, dwContextID, &sendData, sizeof(sendData));
		}
	}
	catch (IDataBaseException * pIException)
	{
		...
		//发送参数
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_GET_GROUP_ROOM_LIST_RESULT, dwContextID, &sendData, sizeof(DBO_GP_GroupRoomListResult));
	}
	return true;
}

//申请入群
bool CDataBaseEngineSink::OnRequestApplyAttachGroup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		OutputDebugString(TEXT("ryp0tdt *** OnRequestApplyAttachGroup"));
		//校验数据
		ASSERT(wDataSize == sizeof(DBO_GP_ApplyAttachGroup));
		if (wDataSize != sizeof(DBO_GP_ApplyAttachGroup)) return false;

		DBO_GP_ApplyAttachGroup* pWriteData = (DBO_GP_ApplyAttachGroup*)pData;

		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@dwGroupID"), pWriteData->dwGroupID);
		m_TreasureDBAide.AddParameter(TEXT("@dwApplyUserID"), pWriteData->dwApplyUserID);
		...
		//结果处理
		if (m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_AddToGroup"), true) == DB_SUCCESS)
		{
			//变量定义
			DBO_GP_ApplyAttachGroupResult sendData;
			ZeroMemory(&sendData, sizeof(DBO_GP_ApplyAttachGroupResult));
			sendData.bSuccessed = true;
			OutputDebugString(TEXT("ryp0tdt *** GSP_GP_AddToGroup 加入成功"));
			//发送结果
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_APPLY_ATTACH_GROUP_RESULT, dwContextID, &sendData, sizeof(DBO_GP_ApplyAttachGroupResult));
		}
		else
		{
			//获取参数
...
//获取申请的群列表
bool CDataBaseEngineSink::OnRequestQueryApplyGroup(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
