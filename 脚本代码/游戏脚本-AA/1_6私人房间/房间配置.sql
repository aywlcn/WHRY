...
WITH ENCRYPTION AS
	...
	DECLARE @cbPayType TINYINT						-- 支付方式 房主支付或AA支付
...
BEGIN
	...
	-- 加载房间
	SELECT @cbCardOrBean = CardOrBean, @cbPayType = PayOwnerOrAA, @lFeeBeanOrRoomCard = FeeBeanOrRoomCard, @cbIsJoinGame = IsJoinGame, @cbMinPeople = MinPeople,
	...
	SELECT @cbCardOrBean AS CardOrBean, @cbPayType AS PayOwnerOrAA, @lFeeBeanOrRoomCard AS FeeBeanOrRoomCard, @cbIsJoinGame AS IsJoinGame, @cbMinPeople AS MinPeople,
...
CREATE PROC GSP_GS_WritePersonalFeeParameter
	...
	@TableFee	INT,					-- 创建花费
	@AATableFee	INT,					-- 创建房间AA花费
	@IniScore	INT,		-- 房间创建后多长时间后还未开始游戏解散桌子
	@strErrorDescribe NVARCHAR(127) OUTPUT		-- 输出信息
WITH ENCRYPTION AS

BEGIN
	INSERT INTO PersonalTableFee (KindID, DrawCountLimit,DrawTimeLimit, TableFee, AATableFee, IniScore ) 
	VALUES (@KindID, @DrawCountLimit,@DrawTimeLimit, @TableFee, @AATableFee, @IniScore  ) 
END
...
BEGIN
	...
	DECLARE @TableFee	INT					-- 创建费用
	DECLARE @AATableFee	INT					-- AA费用
	...
	SELECT  DrawCountLimit, DrawTimeLimit, TableFee, AATableFee, IniScore FROM PersonalTableFee WHERE KindID=@dwKindID
...
BEGIN
	...
	-- 加载房间
	SELECT @cbCardOrBean=CardOrBean, @cbPayType=PayOwnerOrAA, @lFeeBeanOrRoomCard=FeeBeanOrRoomCard,@cbIsJoinGame=IsJoinGame,@cbMinPeople=MinPeople, @cbMaxPeople=MaxPeople, @lPersonalRoomTax =PersonalRoomTax,
	...
	SELECT @cbCardOrBean AS CardOrBean, @cbPayType AS PayOwnerOrAA, @lFeeBeanOrRoomCard AS FeeBeanOrRoomCard,@cbIsJoinGame AS IsJoinGame,@cbMinPeople AS MinPeople, @cbMaxPeople AS MaxPeople, @lPersonalRoomTax AS PersonalRoomTax,
