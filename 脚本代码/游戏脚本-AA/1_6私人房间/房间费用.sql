...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_AATableFeePay]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_AATableFeePay]

IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GR_AATableFeeQuit]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GR_AATableFeeQuit]
...
-- 扣除AA费用
CREATE PROC GSP_GR_AATableFeePay
	@dwUserID INT,								-- 用户 I D
...
BEGIN	
	...
	SELECT  @cbBeanOrRoomCard = CardOrBean, @cbIsJoinGame = IsJoinGame FROM RYPlatformDBLink.RYPlatformDB.dbo.PersonalRoomInfo WHERE KindID = @wKindID 
		...
		--RETURN 1
	-- 查询费用
	SELECT @Fee=AATableFee FROM RYPlatformDBLink.RYPlatformDB.dbo.PersonalTableFee WHERE DrawCountLimit=@dwDrawCountLimit AND DrawTimeLimit=@dwDrawTimeLimit AND KindID = @wKindID
	...
	BEGIN
		...
		BEGIN
			...
			VALUES(@dwUserID,@Fee,1,@dUserBeans,@dUserBeans-@Fee,@strClientIP,GETDATE(),'加入私人房间')	
...
-- 退还AA费用
CREATE PROC GSP_GR_AATableFeeQuit
	@dwUserID INT,									-- 用户 I D
...
BEGIN
	...
	-- 查询费用 及 房主是否参与游戏
	SELECT  @cbBeanOrRoomCard = CardOrBean FROM RYPlatformDBLink.RYPlatformDB.dbo.PersonalRoomInfo WHERE KindID = @wKindID 
...
BEGIN	
	...
	SELECT  @cbBeanOrRoomCard = CardOrBean, @cbIsJoinGame = IsJoinGame FROM RYPlatformDBLink.RYPlatformDB.dbo.PersonalRoomInfo WHERE KindID = @wKindID 
		...
		--RETURN 1
	-- 查询费用
	IF @cbPayType IS NULL OR @cbPayType = 0
	BEGIN
...
BEGIN	
	...
	-- 写入消耗房卡记录
	INSERT INTO RYPlatformDB..StreamCreateTableFeeInfo(UserID,NickName, ServerID, RoomID, CellScore, CardOrBean, CountLimit,TimeLimit, PayOwnerOrAA, CreateTableFee,CreateDate, TaxAgency, JoinGamePeopleCount)
												VALUES(@dwUserID,@Nicname, @dwServerID, @RoomID, @lCellScore,@cbBeanOrRoomCard, @dwDrawCountLimit, @dwDrawTimeLimit, @cbPayType, @Fee,GETDATE(), @dwRoomTax, @wJoinGamePeopleCount)	
...
BEGIN
	...
	-- 加载房间
	SELECT @dwUserID = UserID, @dwPlayTurnCount=CountLimit, @dwPlayTimeLimit = TimeLimit, @sysCreateTime = CreateDate, @sysDissumeTime = DissumeDate, @lTaxCount = TaxRevenue, @cbPayType = PayOwnerOrAA, @lCreateFee = CreateTableFee,
	...
	@sysDissumeTime AS sysDissumeTime, @lTaxCount AS lTaxCount, @cbPayType AS PayOwnerOrAA, @lCreateFee AS CreateTableFee,@cbRoomCardOrBean AS CardOrBean, @bnryRoomScoreInfo AS RoomScoreInfo, @strRoomScoreInfo AS strRoomScoreInfo
